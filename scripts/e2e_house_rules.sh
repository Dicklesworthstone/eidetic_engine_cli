...
#     untagged one, and surfaces it CROSS-WORKSPACE when queried from wsB (the
#     candidate-load union, bd-1n0np.10.1) with originating-workspace provenance.
#   * condition-guarded (no-silent-cap log_drop when the surface is not present
#     in the binary under test): the `house_rule` tag alias is recognized, and
#     the audited promotion CLI `ee remember --scope global` (bd-1n0np.10.2,
#     cli-gated) is exercised when available.
