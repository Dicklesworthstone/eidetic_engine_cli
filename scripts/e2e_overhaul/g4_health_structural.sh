...
if printf '%s' "$HEALTH_JSON" | jq . >/dev/null 2>&1; then
    assert_jq "$HEALTH_JSON" '.schema' "ee.response.v2" "g4_health_structural_response_schema"
    assert_jq "$HEALTH_JSON" '.success' "true" "g4_health_structural_response_success"
    assert_jq "$HEALTH_JSON" 'has("schema") and has("success") and has("data") and (.data | type == "object")' "true" "g4_health_structural_response_required_fields"
    assert_jq "$HEALTH_JSON" '.data.schema' "ee.health.structural.v1" "g4_health_structural_schema"
    assert_jq "$HEALTH_JSON" '(.data | has("schema") and has("snapshotVersion") and has("kTruss") and has("contradictionClusters") and has("summary") and has("degraded"))' "true" "g4_health_structural_required_fields"
    assert_jq "$HEALTH_JSON" '(.data.kTruss | type)' "object" "g4_health_structural_k_truss_object"
    assert_jq "$HEALTH_JSON" '(.data.contradictionClusters | type)' "array" "g4_health_structural_clusters_array"
    assert_jq "$HEALTH_JSON" '(.data.summary | type)' "object" "g4_health_structural_summary_object"
    assert_jq "$HEALTH_JSON" '(.data.degraded | type)' "array" "g4_health_structural_degraded_array"
    assert_jq_nonempty "$HEALTH_JSON" '.data.kTruss.maxK // empty' "g4_health_structural_k_truss_max_k_present"
    K_TRUSS_MAX="$(printf '%s' "$HEALTH_JSON" | jq -r '.data.kTruss.maxK // 0' 2>/dev/null || echo 0)"
    SUPPORT_COUNT="$(printf '%s' "$HEALTH_JSON" | jq -r '.data.kTruss.supportSubgraphMemoryCount // 0' 2>/dev/null || echo 0)"
    CLUSTER_COUNT="$(printf '%s' "$HEALTH_JSON" | jq -r '.data.summary.contradictionClusterCount // 0' 2>/dev/null || echo 0)"
    DENSE_CLUSTER_COUNT="$(printf '%s' "$HEALTH_JSON" | jq '[.data.contradictionClusters[]? | select((.memoryCount // 0) >= 3 and (.contradictionDensity // 0) >= 1)] | length' 2>/dev/null || echo 0)"
    ...
    SNAPSHOT_VERSION=$(printf '%s' "$HEALTH_JSON" | jq -r '.data.snapshotVersion // empty' 2>/dev/null | head -n 1)
else
