...
if [ -f "$CARGO_TOML" ]; then
  ...
  accepted_asupersync_profile=$(grep -E '^asupersync[[:space:]]*=.*default-features = false.*features = \["tracing-integration"\]' "$CARGO_TOML" 2>/dev/null | head -1 || true)
  accepted_asupersync_version=$(printf '%s' "$accepted_asupersync_profile" \
    | sed -n 's/.*version = "=\([^"]*\)".*/\1/p')
  if [ -z "$accepted_asupersync_profile" ] || [ -z "$accepted_asupersync_version" ]; then
    ...
    append_dependency_violation \
      ...
      'an exact asupersync registry pin with default-features=false and tracing-integration'
  fi
fi

if [ -f "$DEPENDENCY_RESEARCH_DOC" ]; then
  append_dependency_checked_file "docs/dependency-research-notes.md"
  if [ -n "$accepted_asupersync_profile" ] \
    && ! grep -Fq "$accepted_asupersync_profile" "$DEPENDENCY_RESEARCH_DOC"; then
  ...
  while IFS= read -r hit; do
    ...
    append_dependency_violation \
      ...
      "$(printf '%s' "$hit" | cut -d: -f2-)" \
      "asupersync =${accepted_asupersync_version} with default-features=false and tracing-integration"
  done < <(grep -nE 'asupersync[^[:cntrl:]]*[0-9]+\.[0-9]+\.[0-9]+|[0-9]+\.[0-9]+\.[0-9]+[^[:cntrl:]]*asupersync' "$DEPENDENCY_RESEARCH_DOC" 2>/dev/null \
    | grep -Fv "$accepted_asupersync_version" || true)
fi
...
if [ -f "$DEPENDENCY_MATRIX_DOC" ]; then
  ...
  if [ -z "$accepted_asupersync_version" ] \
    || ! printf '%s' "$matrix_line" | grep -Fq "registry \`=${accepted_asupersync_version}\`" \
    ...
    append_dependency_violation \
      ...
      "asupersync matrix row includes registry \`=${accepted_asupersync_version}\`, \`default-features = false\`, and \`tracing-integration\`"
  fi
fi
...
if [ -f "$COMPREHENSIVE_PLAN_DOC" ]; then
  ...
  if [ -z "$accepted_asupersync_version" ] \
    || ! printf '%s' "$plan_line" | grep -Fq "version = \"=${accepted_asupersync_version}\"" \
  ...
  while IFS= read -r hit; do
    ...
    append_dependency_violation \
      ...
      "$(printf '%s' "$hit" | cut -d: -f2-)" \
      "asupersync =${accepted_asupersync_version} with default-features=false and tracing-integration"
  done < <(grep -nE 'asupersync[^[:cntrl:]]*[0-9]+\.[0-9]+\.[0-9]+|[0-9]+\.[0-9]+\.[0-9]+[^[:cntrl:]]*asupersync' "$COMPREHENSIVE_PLAN_DOC" 2>/dev/null \
    | grep -Fv "$accepted_asupersync_version" || true)
fi
