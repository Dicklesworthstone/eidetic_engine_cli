...
accepted_asupersync_profile='asupersync = { version = "=0.3.9", default-features = false, features = ["tracing-integration"] }'
...
if [ -f "$DEPENDENCY_RESEARCH_DOC" ]; then
  ...
  while IFS= read -r hit; do
    ...
    append_dependency_violation \
      ...
      "$(printf '%s' "$hit" | cut -d: -f2-)" \
      "asupersync =0.3.9 with default-features=false and tracing-integration"
  done < <(grep -nE 'asupersync[^[:cntrl:]]*(0\.3\.[1-8])|0\.3\.[1-8][^[:cntrl:]]*asupersync|leave .{0,80}asupersync.{0,80}default features|\["test-internals", "proc-macros"\]' "$DEPENDENCY_RESEARCH_DOC" 2>/dev/null || true)
fi
...
if [ -f "$DEPENDENCY_MATRIX_DOC" ]; then
  ...
  if ! printf '%s' "$matrix_line" | grep -Fq "registry \`=0.3.9\`" \
    ...
    append_dependency_violation \
      ...
      "asupersync matrix row includes registry \`=0.3.9\`, \`default-features = false\`, and \`tracing-integration\`"
  fi
fi
...
if [ -f "$COMPREHENSIVE_PLAN_DOC" ]; then
  ...
  if ! printf '%s' "$plan_line" | grep -Fq 'version = "=0.3.9"' \
  ...
  while IFS= read -r hit; do
    ...
    append_dependency_violation \
      ...
      "$(printf '%s' "$hit" | cut -d: -f2-)" \
      "asupersync =0.3.9 with default-features=false and tracing-integration"
  done < <(grep -nE 'asupersync[^[:cntrl:]]*(0\.3\.[1-8]|version = "0\.3")|features = \["proc-macros"\]' "$COMPREHENSIVE_PLAN_DOC" 2>/dev/null || true)
fi
