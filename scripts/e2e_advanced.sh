...
# learning, causal credit commands, daemon recovery, multi-agent writers,
# and backup/restore round-trip coverage.
#
# Usage:
...
#   ./scripts/e2e_advanced.sh daemon_recovery # Run only daemon recovery scenario
#   ./scripts/e2e_advanced.sh multi_agent_writers # Run only multi-agent writer scenario
#   ./scripts/e2e_advanced.sh embedding_native_rerank # Run only rerank fallback scenario
...
# shellcheck disable=SC2329 # Invoked dynamically via scenario_${scenario}.
scenario_daemon_recovery() {
    log_step "Running scenario: daemon_recovery"
    local passed=0
    local failed=0
    local scenario="daemon_recovery"
    local step="cargo_test"
    local stdout_file="${ARTIFACTS_DIR}/${scenario}_${step}_stdout.txt"
    local stderr_file="${ARTIFACTS_DIR}/${scenario}_${step}_stderr.txt"
    local step_start
    ...
    env \
        ...
        RUST_BACKTRACE=1 \
        cargo test --manifest-path "${REPO_ROOT}/Cargo.toml" \
            --test e2e_daemon_recovery -- --nocapture \
        >"${stdout_file}" 2>"${stderr_file}" || exit_code=$?
    ...
    {
        echo "## ${scenario}/${step}"
        echo "Command: cargo test --manifest-path ${REPO_ROOT}/Cargo.toml --test e2e_daemon_recovery -- --nocapture"
        echo "Exit code: ${exit_code}"
        echo "Elapsed: ${elapsed}ms"
        echo "Stdout: ${stdout_file}"
        echo "Stderr: ${stderr_file}"
    ...
    if [[ "${exit_code}" -eq 0 ]]; then
        ((passed++))
        log_pass "cargo test --test e2e_daemon_recovery"
    else
        ((failed++))
        log_fail "cargo test --test e2e_daemon_recovery failed with exit ${exit_code}"
        log_fail "stdout: ${stdout_file}"
        log_fail "stderr: ${stderr_file}"
    fi
...
# shellcheck disable=SC2329 # Invoked dynamically via scenario_${scenario}.
scenario_multi_agent_writers() {
    log_step "Running scenario: multi_agent_writers"
    local passed=0
    local failed=0
    local scenario="multi_agent_writers"
    local step="cargo_test"
    local stdout_file="${ARTIFACTS_DIR}/${scenario}_${step}_stdout.txt"
    local stderr_file="${ARTIFACTS_DIR}/${scenario}_${step}_stderr.txt"
    local step_start
    ...
    env \
        HOME="${TEST_HOME}" \
        EE_TRACE_BEAD_ID="bd-3usjw.57" \
        EE_TEST_TRACE=1 \
        EE_TEST_SEED=42 \
        RUST_BACKTRACE=1 \
        cargo test --manifest-path "${REPO_ROOT}/Cargo.toml" \
            --test e2e_multi_agent_writers -- --nocapture \
        >"${stdout_file}" 2>"${stderr_file}" || exit_code=$?
    ...
    {
        echo "## ${scenario}/${step}"
        echo "Command: EE_TEST_SEED=42 cargo test --manifest-path ${REPO_ROOT}/Cargo.toml --test e2e_multi_agent_writers -- --nocapture"
        echo "Exit code: ${exit_code}"
        echo "Elapsed: ${elapsed}ms"
        echo "Stdout: ${stdout_file}"
        echo "Stderr: ${stderr_file}"
    ...
    if [[ "${exit_code}" -eq 0 ]]; then
        ((passed++))
        log_pass "cargo test --test e2e_multi_agent_writers"
    else
        ((failed++))
        log_fail "cargo test --test e2e_multi_agent_writers failed with exit ${exit_code}"
        log_fail "stdout: ${stdout_file}"
        log_fail "stderr: ${stderr_file}"
    fi
...
scenario_embedding_native_rerank() {
    log_step "Running scenario: embedding_native_rerank"
    local passed=0
    local failed=0
    local scenario="embedding_native_rerank"
    local step="script"
    local stdout_file="${ARTIFACTS_DIR}/${scenario}_${step}_stdout.txt"
    local stderr_file="${ARTIFACTS_DIR}/${scenario}_${step}_stderr.txt"
    local log_dir="${ARTIFACTS_DIR}/${scenario}_logs"
    local step_start
    step_start=$(now_ms)
    local exit_code=0

    mkdir -p "${TEST_WORKSPACE}/tmp" "${log_dir}"

    env \
        HOME="${TEST_HOME}" \
        EE_BIN="${EE_BINARY}" \
        EE_E2E_TMPDIR="${TEST_WORKSPACE}/tmp" \
        EE_E2E_KEEP=1 \
        EE_E2E_KEEP_ARTIFACTS=1 \
        LOG_DIR="${log_dir}" \
        EE_TEST_LOG_PATH="${log_dir}/events.jsonl" \
        "${REPO_ROOT}/scripts/e2e_embedding_native_rerank.sh" \
        >"${stdout_file}" 2>"${stderr_file}" || exit_code=$?
    ...
    {
        echo "## ${scenario}/${step}"
        echo "Command: scripts/e2e_embedding_native_rerank.sh"
        echo "Exit code: ${exit_code}"
        echo "Elapsed: ${elapsed}ms"
        echo "Stdout: ${stdout_file}"
        echo "Stderr: ${stderr_file}"
        echo "Structured log: ${log_dir}/events.jsonl"
        echo ""
    } >> "${LOG_FILE}"

    if [[ "${exit_code}" -eq 0 ]]; then
        ((passed++))
        log_pass "scripts/e2e_embedding_native_rerank.sh"
    else
        ((failed++))
        log_fail "scripts/e2e_embedding_native_rerank.sh failed with exit ${exit_code}"
        log_fail "stdout: ${stdout_file}"
        log_fail "stderr: ${stderr_file}"
        log_fail "structured log: ${log_dir}/events.jsonl"
    fi
