...
if [ -z "$EE_BIN" ]; then
    printf 'e2e_timeline: refusing to run without an explicit binary.\n' >&2
    printf 'e2e_timeline: set EE_BIN (or EE_BINARY) to the ee binary under test.\n' >&2
    exit 2
fi
export EE_BIN
...
if ! ee_require_current_binary "$EE_BIN" "e2e_timeline"; then
    exit 2
fi
