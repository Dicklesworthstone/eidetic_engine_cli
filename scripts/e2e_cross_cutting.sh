...
assert_jq_file "$MIGRATION_MANIFEST" \
    '.currentLastCompiledMigration == 86 and .nextPlannedMigration == 87' \
    "migration registry pins compiled tail and next planned migration"
assert_jq_file "$MIGRATION_MANIFEST" \
    '([.transitionMatrix[].version] | sort) == [66,67,68,69,69,70,71,72,87,88,89] and ([.transitionMatrix[].id] | unique | length) == (.transitionMatrix | length) and ([.transitionMatrix[] | select(.status == "planned") | .version] | unique | length) == ([.transitionMatrix[] | select(.status == "planned")] | length)' \
