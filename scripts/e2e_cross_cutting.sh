...
assert_jq_file "$MIGRATION_MANIFEST" \
    '.currentLastCompiledMigration == 87 and .nextPlannedMigration == 88' \
    "migration registry pins compiled tail and next planned migration"
assert_jq_file "$MIGRATION_MANIFEST" \
    '([.transitionMatrix[].version] | sort) == [66,67,68,69,69,70,71,72,88,89,90] and ([.transitionMatrix[].id] | unique | length) == (.transitionMatrix | length) and ([.transitionMatrix[] | select(.status == "planned") | .version] | unique | length) == ([.transitionMatrix[] | select(.status == "planned")] | length)' \
