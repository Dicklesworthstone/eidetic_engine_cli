...
assert_jq_file "$MIGRATION_MANIFEST" \
    '.currentLastCompiledMigration == 96 and .nextPlannedMigration == 97 and (.policy.nonInitiativeCompiledMigrations.versions.V096 | startswith("V096_MEMORY_SENTINEL_POLARITY"))' \
    "migration registry pins compiled tail and next planned migration"
assert_jq_file "$MIGRATION_MANIFEST" \
    '([.transitionMatrix[].version] | sort) == [66,67,68,69,69,70,71,72,97,98,99] and ([.transitionMatrix[].id] | unique | length) == (.transitionMatrix | length) and ([.transitionMatrix[] | select(.status == "planned") | .version] | unique | length) == ([.transitionMatrix[] | select(.status == "planned")] | length)' \
