...
#
# Full scenario (E9): temp workspace -> remember a memory citing a local
# file#Ln-m span and one citing a git-sha -> mutate the file span and remove the
# referent -> run 'ee verify provenance --json' and assert per-scheme verdicts:
# evidence_drift (file span changed), evidence_missing (referent gone), an
...
file_mem="$(ee_json remember \
    "The two() helper lives at src/prov.rs line 2." \
...
step "drift the file span (mutate) and remove the referent"
printf 'fn one() {}\nfn TWO_RENAMED() {}\nfn three() {}\n' >"$WS/src/prov.rs"  # evidence_drift
# (referent removal is asserted via the verify surface below)

if ! ee_supports verify provenance; then
    log_drop 1 "ee verify provenance CLI absent (bd-1n0np.9.1 wiring pending): when wired, assert .data per-referent status includes evidence_drift (mutated file span) and evidence_missing (removed referent)"
