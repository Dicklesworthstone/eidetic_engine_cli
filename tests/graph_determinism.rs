...
// The `context_pack_dna_output_is_deterministic` and
// `context_pack_dna_pipeline_parity_preserves_pack_fields` tests were removed
// together with the deprecated `ee context` command. They drove Pack DNA
// through `ee context --explain` / `--no-pack-dna`, flags that the replacement
// `ee pack` surface does not expose (it never emits `data.pack.packDna`), so
// the CLI has no surface left to exercise the Pack DNA determinism/parity
// contract. Pack DNA assembly remains covered by the graph-analytics unit and
// contract suites.
