...
- **Tracking Beads:** `bd-3tprq.1` defines the matrix; `bd-3tprq.2`
  implements the parser/static harness.
...
| `WIN-PS1-003` | MUST | README and script examples download the release asset to a file with `Invoke-WebRequest -OutFile`; they must not document `iwr ... .Content \| iex`. GitHub serves release assets as octet streams, so `.Content` is a byte array on Windows. | `install.ps1:56-60`, `README.md` Windows install snippet | Offline docs/script check in `scripts/windows-installer-static-check.ps1` requiring `-OutFile` and forbidding executable byte-array release-asset examples. | No | Covered for README and installer examples; broader docs/help drift remains in `bd-3tprq.5`. |
