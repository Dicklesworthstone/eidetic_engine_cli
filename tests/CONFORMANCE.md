...
| `WIN-PS1-004` | MUST | Default release installs verify SHA256 unless `-NoVerify` or `EE_SKIP_VERIFY=1` is set. SHA256 is the mandatory integrity gate for normal Windows installs. | `install.ps1:7-8`, `install.ps1:42-48`, `install.ps1:90-94`, `install.ps1:901-915` | Mocked download/install flow in `scripts/windows-installer-mocked-flow-check.ps1` asserting the checksum path runs by default. | Mockable offline; final proof on Windows | Covered by the `windows-installer-mocked-flow-conformance` job. |
| `WIN-PS1-005` | MUST | `-NoVerify` skips both SHA256 and Sigstore checks. This escape hatch must be explicit and testable because it weakens integrity verification. | `install.ps1:42-48`, `install.ps1:901-938` | Mocked install flow in `scripts/windows-installer-mocked-flow-check.ps1` asserting no checksum success marker and no fake `cosign` invocation. | Mockable offline; final proof on Windows | Covered by the `windows-installer-mocked-flow-conformance` job. |
| `WIN-PS1-006` | SHOULD | Sigstore/SLSA verification is opt-in by default, after SHA256 succeeds. A missing bundle or missing `cosign` warns by default rather than failing the install. | `install.ps1:8-11`, `install.ps1:45-48`, `install.ps1:916-936` | Mocked missing-bundle and missing-cosign flows in `scripts/windows-installer-mocked-flow-check.ps1`. | Mockable offline; final proof on Windows | Intentional POSIX parity: fatal provenance is only `-RequireProvenance` or `EE_REQUIRE_PROVENANCE=1`. |
| `WIN-PS1-007` | MUST | `-RequireProvenance` fails when the Sigstore bundle is unavailable, and never silently degrades to an unverified source build: it is fatal both when combined with `-FromSource` and when the signed-artifact download fails (POSIX parity with `install.sh:1125,1209`). Required provenance cannot silently degrade. | `install.ps1:45-48`, `install.ps1:836-838`, `install.ps1:898-900`, `install.ps1:932-939` | Mocked bundle download failure with `-RequireProvenance` in `scripts/windows-installer-mocked-flow-check.ps1`. | Mockable offline; final proof on Windows | Covered by the `windows-installer-mocked-flow-conformance` job. |
...
| `WIN-PS1-013` | MUST | Release packaging smoke must prove the default Windows installer can reach semantic readiness on first use when explicitly opted in with `EE_INSTALL_SEMANTIC_SMOKE=require`; ordinary installer verification must remain concise and must not download the model by default. | `install.ps1:653-718`, `scripts/windows-installer-live-smoke.ps1:33`, `scripts/windows-installer-live-smoke.ps1:185`, `scripts/windows-installer-live-smoke.ps1:258-300`, `docs/testing-strategy.md` release installer semantic smoke section | `scripts/windows-installer-live-smoke.ps1` sets `EE_INSTALL_SEMANTIC_SMOKE=require`, installs the release binary, initializes a temporary workspace, remembers a semantic fact, rebuilds the index, and logs a `semantic_model_status` `ee.test_event.v1` event whose `semanticReadiness.state` is `available` and `mode` is `semantic`. | Yes for live path; network required | Covered by the skipped-by-default live smoke and the release installer smoke. Static conformance pins the opt-in variable, semantic phase events, `selected_model_id`, readiness vocabulary, and installer warn/require failure semantics so this row cannot silently disappear. |
| `WIN-PS1-014` | MUST | A matching-version rerun remains download/build/lock-free but still repairs the user `PATH`, regenerates completions, and honors `-Verify`. Native `ee.exe --version` execution or nonzero exit is fatal; a nonzero first-run `ee doctor --json` posture remains an explicit warning. | `install.ps1` `Test-InstalledVersion`, `Install-Completions`, `Invoke-SelfTest`, and the matching-version branch in `Main` | AST-backed source assertions in `scripts/windows-installer-static-check.ps1` pin native exit-code handling and require PATH/completion/verification calls before the lock boundary. The mocked flow harness continues to cover the surrounding no-Rust installer execution contract. | Yes | Covered by `windows-installer-static-conformance` under Windows PowerShell 5.1 and PowerShell 7+, plus `windows-installer-mocked-flow-conformance` for the adjacent release-install flow. |
| `WIN-PS1-015` | MUST | Installer summaries and agent-integration guidance introduce new users to canonical `ee pack`; they must not steer users toward the soft-deprecated `ee context` alias. | `install.ps1` `Show-AgentIntegration` and `Show-Summary` | Static source assertion in `scripts/windows-installer-static-check.ps1` requires `ee pack` and rejects `ee context`; the cross-platform Rust installer contract applies the same rule to both installers. | Yes | Covered by `windows-installer-static-conformance` under Windows PowerShell 5.1 and PowerShell 7+, plus `installers_recommend_the_canonical_pack_surface`. |
...
- `bd-3tprq.3` mocked-flow checks live in
  `scripts/windows-installer-mocked-flow-check.ps1`. The harness serves local
