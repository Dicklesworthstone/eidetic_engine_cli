...
| `WIN-PS1-007` | MUST | `-RequireProvenance` fails when the Sigstore bundle is unavailable. Required provenance cannot silently degrade. | `install.ps1:45-48`, `install.ps1:923-930` | Mocked bundle download failure with `-RequireProvenance` in `scripts/windows-installer-mocked-flow-check.ps1`. | Mockable offline; final proof on Windows | Covered by the `windows-installer-mocked-flow-conformance` job. |
