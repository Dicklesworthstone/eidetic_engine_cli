//! bd-2wr4f: real-binary pin test for `ee graph pagerank`.
//!
//! Mirrors the runtime shape of `graph_neighborhood_smoke.rs` and
//! `e2e_graph_path.rs` but exercises CLI angles the existing graph tests
//! do not assert against the real `ee` binary:
//!
...
//!   empty `scores` array and a well-formed surface envelope
//! * a directed two-edge path src -> mid -> dst surfaces `status=computed`
//!   with `nodeCount=3`, `edgeCount=2`, scores sorted by descending score
//!   then ascending memoryId, ranks starting at 1, and a witness whose
//!   `edgesScanned > 0`
    ...
    connection
        .insert_memory_link(
            ...
            &CreateMemoryLinkInput {
                ...
                created_by: Some("e2e-graph-pagerank-pin".to_string()),
                metadata_json: None,
            },
        )
...
fn run_graph_pagerank(workspace_arg: &str, extra: &[&str]) -> Result<(Output, Value), String> {
...
fn assert_envelope_shape(data: &Value) -> TestResult {
        ...
        data["command"].as_str() == Some("graph pagerank"),
        format!("command must be graph pagerank; got {data}"),
    )?;
    ensure(
...
fn seed_two_edge_path() -> Result<(PathBuf, String, String, String, String), String> {
    ...
    let src = remember(&workspace_arg, "Pin-test pagerank src node.")?;
    let mid = remember(&workspace_arg, "Pin-test pagerank mid node.")?;
    let dst = remember(&workspace_arg, "Pin-test pagerank dst node.")?;
...
#[test]
fn graph_pagerank_rejects_zero_limit_with_usage_error() -> TestResult {
    ...
    let (output, parsed) = run_graph_pagerank(&workspace_arg, &["--limit", "0"])?;
    ensure(
        !output.status.success(),
        format!(
            "graph pagerank --limit 0 must fail; stdout: {}",
            String::from_utf8_lossy(&output.stdout)
        ),
    )?;
    assert_usage_error(
...
#[test]
fn graph_pagerank_rejects_min_weight_out_of_range_with_usage_error() -> TestResult {
    ...
    let (output, parsed) = run_graph_pagerank(&workspace_arg, &["--min-weight", "2.0"])?;
    ensure(
        !output.status.success(),
        format!(
            "graph pagerank --min-weight 2.0 must fail; stdout: {}",
            String::from_utf8_lossy(&output.stdout)
        ),
    )?;
    assert_usage_error(
...
#[test]
fn graph_pagerank_rejects_min_confidence_out_of_range_with_usage_error() -> TestResult {
    ...
    let (output, parsed) = run_graph_pagerank(&workspace_arg, &["--min-confidence", "-0.5"])?;
        ...
        !output.status.success(),
        format!(
            "graph pagerank --min-confidence -0.5 must fail; stdout: {}",
            String::from_utf8_lossy(&output.stdout)
        ),
    )?;
    assert_usage_error(
...
#[test]
fn graph_pagerank_returns_empty_scores_on_fresh_workspace() -> TestResult {
    ...
    let (output, parsed) = run_graph_pagerank(&workspace_arg, &[])?;
    ensure(
        output.status.success(),
        format!(
            "graph pagerank on a fresh workspace must exit zero; stderr: {}",
            String::from_utf8_lossy(&output.stderr)
        ),
    )?;
    ensure(
...
#[test]
fn graph_pagerank_returns_ranked_scores_for_two_edge_path() -> TestResult {
    ...
    let (output, parsed) = run_graph_pagerank(&workspace_arg, &[])?;
        ...
        output.status.success(),
        format!(
            "graph pagerank with seeded links must succeed; stderr: {}",
            String::from_utf8_lossy(&output.stderr)
        ),
    )?;
    let data = &parsed["data"];
    ...
    // (the deterministic order graph_scores_json constructs). Verify both
    // orderings hold across adjacent pairs.
...
#[test]
fn graph_pagerank_limit_truncates_deterministically() -> TestResult {
    let (_workspace, workspace_arg, _src, _mid, _dst) = seed_two_edge_path()?;

    let (unbounded_output, unbounded_parsed) = run_graph_pagerank(&workspace_arg, &[])?;
    ensure(
        unbounded_output.status.success(),
        "unbounded graph pagerank must succeed".to_string(),
    )?;
    ...
    let (limited_output, limited_parsed) = run_graph_pagerank(&workspace_arg, &["--limit", "1"])?;
    ensure(
        limited_output.status.success(),
        "graph pagerank --limit 1 must succeed".to_string(),
    )?;
