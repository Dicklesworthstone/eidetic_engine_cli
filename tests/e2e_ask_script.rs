...
#[test]
fn ask_e2e_script_exercises_public_cli() {
    let repo = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let ee_bin = PathBuf::from(env!("CARGO_BIN_EXE_ee"));
    let scratch = retained_scratch_dir();
    let log_dir = scratch.join("logs");
    fs::create_dir_all(&log_dir).expect("create retained ask e2e log directory");

    let output = Command::new(repo.join("scripts/e2e_ask.sh"))
        .current_dir(&repo)
        .env("EE_BIN", &ee_bin)
        .env("EE_E2E_KEEP", "1")
        ...
        .output()
        .expect("run scripts/e2e_ask.sh");

    if !output.status.success() {
        panic!(
            "scripts/e2e_ask.sh failed with status {:?}\nstdout:\n{}\nstderr:\n{}",
            output.status.code(),
    ...
    let summary_bytes = fs::read(&summary_path).unwrap_or_else(|error| {
        panic!(
            "read ask e2e summary at {}: {error}",
            summary_path.display()
        )
    });
    let summary: serde_json::Value =
        serde_json::from_slice(&summary_bytes).expect("parse ask e2e summary JSON");

    assert_eq!(summary["schema"], "ee.test_event.v1.summary");
    assert_eq!(summary["test"], "ask");
    assert_eq!(summary["verdict"], "PASS");
    assert!(
        summary["pass"].as_u64().unwrap_or(0) >= 20,
        "ask e2e should record substantial public-CLI assertions: {summary}"
    );
