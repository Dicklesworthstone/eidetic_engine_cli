//! bd-1g164: real-binary pin test for `ee graph louvain`.
//!
//! Mirrors the runtime shape of `graph_neighborhood_smoke.rs` and the
//! sibling pin tests for pagerank (bd-2wr4f), betweenness (bd-2r363),
//! hits (bd-177gh), path (bd-3vf6j), and centrality (bd-1duvm), but
//! exercises CLI angles unique to louvain:
//!
//! * three louvain-specific usage errors from
//!   `validate_graph_louvain_args` (resolution, threshold, max-level)
//!   layered on top of the shared `validate_graph_read_options`
//!   surfaces (min-weight, min-confidence, limit).
//! * empty workspace returns `status=computed` with `communityCount=0`
//!   and an empty `communities` array under the
//!   `ee.graph.algorithm.v1` envelope and `command="graph louvain"`.
//! * two disjoint two-node components (a -> b and c -> d) yield two
//!   communities of size 2 each, with community ids `community_0001`
//!   / `community_0002`, communities sorted by descending size then
//!   ascending first-node id, and intra-community nodes sorted
//!   alphabetically.
//! * `--seed 42` produces byte-identical output across repeated runs
    ...
    connection
        .insert_memory_link(
            ...
            &CreateMemoryLinkInput {
                ...
                created_by: Some("e2e-graph-louvain-pin".to_string()),
                metadata_json: None,
            },
        )
...
fn run_graph_louvain(workspace_arg: &str, extra: &[&str]) -> Result<(Output, Value), String> {
    let mut args: Vec<&str> = vec!["--workspace", workspace_arg, "--json", "graph", "louvain"];
    args.extend_from_slice(extra);
    let output = run_ee(&args)?;
    let parsed: Value = serde_json::from_slice(&output.stdout)
        .map_err(|error| format!("graph louvain stdout must be JSON: {error}"))?;
    Ok((output, parsed))
}
...
fn assert_envelope_shape(data: &Value) -> TestResult {
        ...
        data["command"].as_str() == Some("graph louvain"),
        format!("command must be graph louvain; got {data}"),
    )?;
    ensure(
...
-> Result<(PathBuf, String, String, String, String, String), String> {
    ...
    let a = remember(&workspace_arg, "Pin-test louvain component A node 1.")?;
    let b = remember(&workspace_arg, "Pin-test louvain component A node 2.")?;
...
#[test]
fn graph_louvain_rejects_negative_resolution_with_usage_error() -> TestResult {
    let workspace = unique_workspace("usage-resolution")?;
    let workspace_arg = workspace
        .to_str()
        .ok_or_else(|| "workspace path must be UTF-8".to_string())?
        .to_owned();
    ...
    let (output, parsed) = run_graph_louvain(&workspace_arg, &["--resolution", "-1.0"])?;
        ...
        format!(
            "graph louvain --resolution -1.0 must fail; stdout: {}",
            String::from_utf8_lossy(&output.stdout)
        ),
    )?;
    assert_usage_error(
        &parsed,
        &["--resolution must be a finite positive value"],
        "Use a value like 1.0.",
    )
}

#[test]
fn graph_louvain_rejects_negative_threshold_with_usage_error() -> TestResult {
    let workspace = unique_workspace("usage-threshold")?;
    let workspace_arg = workspace
        .to_str()
        .ok_or_else(|| "workspace path must be UTF-8".to_string())?
        .to_owned();
    ...
    let (output, parsed) = run_graph_louvain(&workspace_arg, &["--threshold", "-1.0"])?;
        ...
        !output.status.success(),
        format!(
            "graph louvain --threshold -1.0 must fail; stdout: {}",
            String::from_utf8_lossy(&output.stdout)
        ),
    )?;
    assert_usage_error(
        ...
        "Use a value like 1e-7.",
    )
}

#[test]
fn graph_louvain_rejects_zero_max_level_with_usage_error() -> TestResult {
    let workspace = unique_workspace("usage-max-level")?;
    let workspace_arg = workspace
        .to_str()
        .ok_or_else(|| "workspace path must be UTF-8".to_string())?
        .to_owned();
    ...
    let (output, parsed) = run_graph_louvain(&workspace_arg, &["--max-level", "0"])?;
    ensure(
        !output.status.success(),
        format!(
            "graph louvain --max-level 0 must fail; stdout: {}",
            String::from_utf8_lossy(&output.stdout)
        ),
    )?;
    assert_usage_error(
        &parsed,
        &["--max-level must be greater than zero"],
        "Omit --max-level or pass a positive value.",
    )
}

#[test]
fn graph_louvain_rejects_min_weight_out_of_range_with_usage_error() -> TestResult {
    let workspace = unique_workspace("usage-min-weight")?;
    let workspace_arg = workspace
        .to_str()
        .ok_or_else(|| "workspace path must be UTF-8".to_string())?
        .to_owned();
    ...
    let (output, parsed) = run_graph_louvain(&workspace_arg, &["--min-weight", "2.0"])?;
    ensure(
        !output.status.success(),
        format!(
            "graph louvain --min-weight 2.0 must fail; stdout: {}",
            String::from_utf8_lossy(&output.stdout)
        ),
    )?;
    assert_usage_error(
...
#[test]
fn graph_louvain_returns_empty_communities_on_fresh_workspace() -> TestResult {
    ...
    let (output, parsed) = run_graph_louvain(&workspace_arg, &[])?;
    ensure(
        output.status.success(),
        format!(
            "graph louvain on a fresh workspace must exit zero; stderr: {}",
            String::from_utf8_lossy(&output.stderr)
        ),
    )?;
    ensure(
...
#[test]
fn graph_louvain_splits_disjoint_components_into_separate_communities() -> TestResult {
    ...
    let (output, parsed) = run_graph_louvain(&workspace_arg, &[])?;
        ...
        output.status.success(),
        format!(
            "graph louvain on disjoint components must succeed; stderr: {}",
            String::from_utf8_lossy(&output.stderr)
        ),
    )?;
    let data = &parsed["data"];
    ...
    // Each community has size 2 and carries one src+dst pair. Verify
    // exactly: union of communities == {a,b,c,d}; intersection of
    // communities == empty; one community contains {a,b} and the
    // other contains {c,d}.
    ...
    for (index, community) in communities.iter().enumerate() {
        ...
        // Intra-community nodes must be sorted alphabetically.
...
#[test]
fn graph_louvain_seeded_runs_are_byte_deterministic() -> TestResult {
    let (_workspace, workspace_arg, _a, _b, _c, _d) = seed_two_disjoint_components()?;

    let first = run_graph_louvain(&workspace_arg, &["--seed", "42"])?.0;
    let second = run_graph_louvain(&workspace_arg, &["--seed", "42"])?.0;
    ensure(
        first.status.success() && second.status.success(),
        "both seeded runs must succeed".to_string(),
    )?;
    ...
    let second_stdout = String::from_utf8_lossy(&second.stdout).into_owned();
        ...
        format!(
            "louvain --seed 42 stdout must be byte-deterministic across runs; first={first_stdout:?} second={second_stdout:?}"
        ),
    )?;
    Ok(())
}
