...
fn single_lf_terminated_line_reports_exact_counts_and_peaks() -> TestResult {
    ...
    // line.len() reports 5 (the trailing LF is stripped), so:
    //   line_count        = 1
    //   bytes_seen        = 5
    //   peak_line_bytes   = 5
...
fn multiple_lines_sum_bytes_and_report_largest_line() -> TestResult {
    // Three lines with distinct lengths (1, 4, 2) yield:
    //   line_count        = 3
    //   bytes_seen        = 1 + 4 + 2 = 7
...
fn crlf_line_endings_are_stripped_before_counting() -> TestResult {
    // "abc\r\n" should count as a 3-byte line (both \r and \n
    // stripped). peak_buffer_bytes still captures buf.len()=5 because
...
fn trailing_line_without_newline_is_still_counted() -> TestResult {
    // "first\nsecond" should yield two lines: "first" (5 bytes, LF
    // stripped) and "second" (6 bytes, no trailing delimiter). peak
    // values track the second line which is longer.
    let summary = decode(b"first\nsecond")?;
    ensure_equal(&summary.line_count, &2, "trailing-no-newline line count")?;
    ensure_equal(
        &summary.bytes_seen,
        &11,
        "trailing-no-newline bytes_seen (5 + 6)",
    )?;
    ensure_equal(
...
#[test]
fn empty_lines_contribute_zero_bytes_but_increment_line_count() -> TestResult {
    // "\n\n\n" is three empty lines: line.len()=0 each. Summary
    // should reflect line_count=3, all byte/peak fields zero except
    // peak_buffer_bytes which captures the single LF byte.
    let summary = decode(b"\n\n\n")?;
    ensure_equal(
        &summary,
        &CassStdoutDecodeFuzzSummary {
