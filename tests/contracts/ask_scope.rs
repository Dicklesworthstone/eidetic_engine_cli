...
fn scoped(workspace: &Path, scope: &str, actor: Option<&str>, question: &str) -> Result<Value, String> {
    let output = crate::common_spawn::serialized_real_ee_with(|command| {
        ...
        command.arg("--json").arg("--workspace").arg(workspace)
            .arg("ask").arg(question).arg("--memory-scope").arg(scope);
    }).map_err(|error| error.to_string())?;
    if !output.status.success() {
        return Err(format!("scoped ask failed: {}", String::from_utf8_lossy(&output.stderr)));
    }
...
fn seed_scope(db: &DbConnection, workspace: &str, number: usize, actor: &str, trust: &str, tags: &[&str]) -> Result<String, String> {
    let id = format!("mem_{number:026}");
    db.insert_memory(&id, &CreateMemoryInput {
        workspace_id: workspace.to_owned(), content: "Run cargo fmt before release.".to_owned(),
        ...
        valid_from: Some("2000-01-01T00:00:00Z".to_owned()), valid_to: None,
    }).map_err(|error| error.to_string())?;
    Ok(id)
}
...
fn public_ask_routes_all_memory_scopes_before_evidence_selection() -> Result<(), String> {
    ...
    db.insert_team_member(&InsertTeamMemberInput {
        member_id: format!("mbr_{:032x}", 1), team_id: "team_ask".to_owned(),
        workspace_id: workspace_id.clone(), display_name: "Bob".to_owned(),
        state: "active".to_owned(), is_self: false, origin_node_id: "node_Bob".to_owned(),
        bound_via: "invite_ceremony".to_owned(), joined_at: "2020-01-01T00:00:00Z".to_owned(),
    }).map_err(|error| error.to_string())?;
    drop(db);
    ...
    for (scope, expected, forbidden) in [
        ...
        ("global", 1, vec![alice.as_str(), assertion.as_str(), outsider.as_str()]),
        ("workspace", 4, vec![]),
        ("swarm", 4, vec![]),
    ] {
        ...
        for id in forbidden {
            assert!(!data.to_string().contains(id), "out-of-scope evidence leaked for {scope}");
        }
    ...
    assert!(no_actor["citations"].as_array().ok_or("citations")?.is_empty());
    Ok(())
}
...
fn public_ask_withholds_private_bodies_and_citation_metadata() -> Result<(), String> {
    ...
    let workspace_id = db.get_workspace_by_path(&workspace.to_string_lossy())
        .map_err(|error| error.to_string())?.ok_or("missing workspace")?.id;
    let private = seed(&db, &workspace_id, 1,
        "Run cargo fmt before release using password=ask-cli-canary.", "2000-01-01T00:00:00Z", None)?;
    let safe = seed(&db, &workspace_id, 2,
        "Run cargo fmt before release.", "2000-01-01T00:00:00Z", None)?;
    ...
    assert_eq!(response["data"]["citations"][0]["provenanceUri"], format!("ee://memory/{safe}"));
    Ok(())
}
...
fn public_ask_rejects_invalid_scope_at_the_argument_boundary() -> Result<(), String> {
    ...
    let output = crate::common_spawn::serialized_real_ee_with(|command| {
        command.arg("--workspace").arg(root.path()).arg("ask")
            .arg("release").arg("--memory-scope").arg("unknown-scope");
    }).map_err(|error| error.to_string())?;
