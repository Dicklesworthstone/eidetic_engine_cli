...
const INVENTORY_RULES: &[InventoryRule] = &[
    ...
    // The singles. EVERY fragment below is the finding's own line, chosen by the
    // anchor rule in the match-count ledger rather than by reading the code.
    allowed_in(
        "NSF-CORE-AGENTSMD-ABBREVIATION-WORD",
        "src/core/agentsmd.rs",
        "ends_with_abbreviation",
        ".unwrap_or_default();",
        "UNREACHABLE: str::rsplit always yields at least one element. The empty string would also be harmless -- it has length 0 so the single-uppercase-letter test fails, and it matches none of the abbreviation literals.",
    ),
    allowed_in(
        "NSF-CORE-ASK-NEAREST-EVIDENCE-SAMPLE",
        "src/core/ask.rs",
        "evaluate_ask_inner",
        ".unwrap_or_default();",
        "UNREACHABLE: this is the `else` of `if nearest_evidence.is_empty()`, and the empty case has its own message (\"no memory mentions X; the corpus has no stored evidence\"). So `.first()` is Some here, and the hint never renders an empty sample beside the words \"nearest evidence\".",
    ),
    allowed_in(
        "NSF-CORE-CONFORMAL-CALIBRATION-SCORES",
        "src/core/conformal.rs",
        "load_conformal_nonconformity_scores",
        "read_conformal_calibration(file, CONFORMAL_CALIBRATION_MAX_BYTES).unwrap_or_default()",
        "A read failure yields no residuals, the same as the absent-file path two lines above. That merge is acceptable ONLY because the consumer declares it: why_conformal_confidence_intervals counts the residuals and, below MIN_WHY_CONFORMAL_CALIBRATION_SAMPLES, reports status \"conservative_insufficient_calibration\" with quantile 1.0 -- the widest interval. So an unreadable calibration file produces a conservative, explicitly-labelled answer rather than a confident wrong one. RESIDUAL, stated rather than hidden: an operator cannot distinguish \"no calibration file\" from \"calibration file unreadable\", since both report insufficient calibration.",
    ),
    allowed_in(
        "NSF-CORE-CONTEXT-ZERO-LIMIT",
        "src/core/context.rs",
        "admit_recent_context_memories",
        "return Ok(Vec::new());",
        "A caller asking for zero memories gets zero. The guard is the explicit `if limit == 0` directly above, so this is a requested empty result and not a failed query.",
    ),
    allowed_in(
        "NSF-CORE-JSONL-IMPORT-NO-LINKS",
        "src/core/jsonl_import.rs",
        "prepare_links",
        "return Ok(Vec::new());",
        "An import with no links prepares none, guarded by the explicit `if parsed.links.is_empty()` above. A malformed link takes a different path entirely -- this function returns Vec<JsonlImportError> on the error side.",
    ),
    allowed_in(
        "NSF-CORE-LEARN-PROPOSED-CONTENT-HASH",
        "src/core/learn.rs",
        "propose_experiments",
        "\"proposedContentHash\": blake3::hash(input.proposed_content.as_deref().unwrap_or_default()",
        "A proposal with no content hashes as empty, and that is CONSISTENT rather than fabricated: the verifier recomputes the identical expression (curate's audited_source_memory_ids_for_rule_candidate compares against blake3::hash(stored.proposed_content.as_deref().unwrap_or_default())), so writer and reader agree on what the hash of an absent body is. The hash is a binding between the audit row and the proposal, not an assertion that content existed.",
    ),
    allowed_in(
        "NSF-CORE-MEMORY-DEBT-UNPARSEABLE-AGE",
        "src/core/memory_debt.rs",
        "detect_contradicted",
        ".unwrap_or_default();",
        "A memory whose updated_at will not parse yields age 0, and the guard below (`if age_days < CONTRADICTION_WINDOW_DAYS { return; }`) then SKIPS it. So an undateable memory is not flagged as contradiction debt. The direction is to claim less than the evidence supports, which is the right bias for a debt detector that would otherwise assert staleness it cannot date.",
    ),
    allowed_in(
        "NSF-CORE-OUTCOME-NO-QUARANTINE",
        "src/core/outcome.rs",
        "record_outcome_inner",
        ".unwrap_or_default();",
        "No quarantine means no quarantine degradations to report. The `.map` builds the degradation vector only when a (cause, summary) exists, so the empty vector is the true state rather than a swallowed one. The comment above notes this is the dry-run path, which reports the same shape without persisting candidate ids.",
    ),
    allowed_in(
        "NSF-CORE-PLAN-RECIPE-NAME-FIRST-LINE",
        "src/core/plan.rs",
        "procedural_rule_entry",
        ".unwrap_or_default()",
        "The recipe name is the first line of the rule content, truncated to 120 chars. A rule with empty content has no first line and gets an empty name -- str::lines() yields nothing for an empty string, which is the only way to reach this.",
    ),
    allowed_in(
        "NSF-CORE-TAILSCALE-CHUNK-SIZE-TOKEN",
        "src/core/tailscale_probe.rs",
        "decode_chunked_body",
        "let size_token = size_line.split(';').next().unwrap_or_default().trim();",
        "UNREACHABLE (str::split always yields one element) and safe regardless: the token goes to from_str_radix, whose failure rejects the chunk. The same construct and the same argument as the responder broker's chunk decoder.",
    ),
    allowed_in(
        "NSF-CORE-WORKSPACE-RESERVATION-EXPIRY",
        "src/core/workspace.rs",
        "beads_reservations_from_agent_mail_input",
        "expires_ts_rfc3339: reservation.expires_at.clone().unwrap_or_default(),",
        "A reservation that agent-mail reports without an expiry is projected with an empty expiry string, which reads as \"no expiry recorded\". It is a projection of an upstream record into the beads shape; nothing here computes expiry or compares timestamps, so an empty value cannot make a lease look live.",
    ),
    allowed_in(
        "NSF-DB-PACK-HISTORY-RECOVERY-KEYS",
        "src/db/mod.rs",
        "insert_pack_history_rows_for_recovery",
        "Value::Text(baseline.task_key.clone().unwrap_or_default()), Value::Text(baseline.pack_id",
        "A recovery insert writes the baseline's optional keys as empty text when the baseline carries none. This is the RECOVERY path reconstructing history rows from whatever a damaged baseline still has; writing an empty key preserves the row and its other columns rather than dropping the record, and an empty task key cannot collide with a real one in a lookup.",
    ),
    allowed_in(
        "NSF-HOOKS-HARNESS-COMMAND-TEMPLATE",
        "src/hooks/installer.rs",
        "harness_conformance_event_payload",
        "let command = case.input.command_template.clone().unwrap_or_default();",
        "A conformance case that declares no command template has none, and the payload records that. This builds a describing payload for a harness case, not a command to execute.",
    ),
    allowed_in(
        "NSF-MESH-RESPONDER-AUDIT-EVENTS-LOCK",
        "src/mesh/responder_broker.rs",
        "recent_audit_events",
        ".unwrap_or_default()",
        "The default fires when the runtime mutex is POISONED -- i.e. a thread panicked while holding it -- and returns no recent audit events. Recorded plainly rather than waved through: this UNDER-REPORTS, and a reader of an empty audit list cannot tell a quiet broker from a poisoned one. It is accepted because the accessor returns Vec with no error channel and a poisoned lock is already a reported-elsewhere catastrophic state, but it is the weakest allow in this file.",
    ),
    allowed_in(
        "NSF-MODELS-VERIFICATION-EXECUTABLE-HASH",
        "src/models/verification.rs",
        "validate_v2_artifact_manifest_bindings",
        "executable.unwrap_or_default(),",
        "An absent executable hashes to something that cannot match the advertised command hash, and the result is `.ok_or_else(|| mismatch(..))?`. The advertised hash itself is separately REQUIRED three lines above with ok_or_else(mismatch(\"command_hash is missing\")), so the validator refuses rather than passing on an empty binding.",
    ),
    allowed_in(
        "NSF-OUTPUT-WHY-DEGRADED-REPAIR",
        "src/output/mod.rs",
        "why_degraded_json",
        "entry.repair.as_deref().unwrap_or_default(),",
        "A degradation with no repair command renders an empty repair string; code, severity and message are all supplied explicitly on the lines above. Same reading as the CLI and model degradation-repair rules.",
    ),
    allowed_in(
        "NSF-PACK-CACHED-EMBED-BACKEND",
        "src/pack/mod.rs",
        "cached_context_embed_backend",
        ".unwrap_or_default()",
        "A cached context whose payload carries no embed_backend, or one that does not parse as a known backend, reports the default backend. The `.parse().ok()` does discard a parse failure, which is why this is worth stating: the value is a descriptive label on a CACHED response and nothing selects an embedder from it -- the live path reads its backend from configuration, not from the cache.",
    ),
    allowed_in(
        "NSF-SERVE-RESPONSE-STATUS-LINE",
        "src/serve.rs",
