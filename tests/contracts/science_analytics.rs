...
fn assert_eval_report_payload(payload: &JsonValue, context: &str) -> TestResult {
    ...
    ensure_json_equal(
        payload
            .pointer("/data/report/fixture_id")
    ...
    ensure_json_equal(
        payload
            .pointer("/data/report/metrics/queries_evaluated")
...
#[test]
fn eval_run_science_json_reports_fixture_without_metrics() -> TestResult {
    let payload = parse_json_stdout(
        run_ee(&[
            "--json",
            "eval",
...
fn clustering_diagnostics_json_contract_matches_feature_state() -> TestResult {
    ...
    ensure_json_equal(
        payload
            .get("degradationCode")
            .ok_or("missing clustering diagnostics degradation code")?,
        &JsonValue::Null,
