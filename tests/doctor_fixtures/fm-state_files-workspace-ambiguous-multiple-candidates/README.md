...
2. `assert.sh` confirms the marker is present. When
   ...
   `ee doctor --fix --only fm-state_files-workspace-ambiguous-multiple-candidates`, then a follow-up
...
`ee doctor --fix --only fm-state_files-workspace-ambiguous-multiple-candidates` is WIRED. `bd-3boan` (CLI surface for
