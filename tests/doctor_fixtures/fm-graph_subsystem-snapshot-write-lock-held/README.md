...
2. `assert.sh` confirms the marker is present. When
   ...
   `ee doctor --fix --only fm-graph_subsystem-snapshot-write-lock-held`, then a follow-up
...
`ee doctor --fix --only fm-graph_subsystem-snapshot-write-lock-held` is WIRED. `bd-3boan` (CLI surface for
