...
2. `assert.sh` confirms the marker is present. When
   ...
   `ee doctor --fix --only fm-state_files-jsonl-tombstone-drift`, then a follow-up
...
`ee doctor --fix --only fm-state_files-jsonl-tombstone-drift` is WIRED. `bd-3boan` (CLI surface for
