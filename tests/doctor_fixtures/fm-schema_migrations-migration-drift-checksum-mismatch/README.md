...
2. `assert.sh` confirms the marker is present. When
   ...
   `ee doctor --fix --only fm-schema_migrations-migration-drift-checksum-mismatch`, then a follow-up
...
`ee doctor --fix --only fm-schema_migrations-migration-drift-checksum-mismatch` is WIRED. `bd-3boan` (CLI surface for
