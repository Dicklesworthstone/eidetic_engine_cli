...
2. `assert.sh` confirms the marker is present. When
   ...
   `ee doctor --fix --only fm-agent_coordination-mcp-agent-mail-file-reservation-conflict`, then a follow-up
...
`ee doctor --fix --only fm-agent_coordination-mcp-agent-mail-file-reservation-conflict` is WIRED. `bd-3boan` (CLI surface for
