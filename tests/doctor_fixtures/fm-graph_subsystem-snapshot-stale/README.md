...
| Field | Value |
| --- | --- |
| Failure-mode id | `fm-graph_subsystem-snapshot-stale` |
| Severity | P1 |
| Subsystem | graph_subsystem |
| Repair spec | [`docs/doctor/repair-specs/graph_subsystem.md#fm-graph_subsystem-snapshot-stale`](../../../docs/doctor/repair-specs/graph_subsystem.md#fm-graph_subsystem-snapshot-stale) |
...
1. `corrupt.sh` builds an isolated corrupt workspace at
   ...
   `.ee/doctor-fixtures/fm-graph_subsystem-snapshot-stale.json`, plus a baseline
...
2. `assert.sh` confirms the marker is present. When
   ...
   `ee doctor --fix --only fm-graph_subsystem-snapshot-stale`, then a follow-up
...
`ee doctor --fix --only fm-graph_subsystem-snapshot-stale` is WIRED. `bd-3boan` (CLI surface for
