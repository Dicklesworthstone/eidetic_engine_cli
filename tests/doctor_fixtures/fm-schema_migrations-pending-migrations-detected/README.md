...
2. `assert.sh` confirms the marker is present. When
   ...
   `ee doctor --fix --only fm-schema_migrations-pending-migrations-detected`, then a follow-up
...
`ee doctor --fix --only fm-schema_migrations-pending-migrations-detected` is WIRED. `bd-3boan` (CLI surface for
