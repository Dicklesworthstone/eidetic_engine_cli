...
1. `corrupt.sh` builds an isolated corrupt workspace at
   ...
   `.ee/doctor-fixtures/fm-graph_subsystem-snapshot-missing.json`, plus a baseline
...
2. `assert.sh` confirms the marker is present. When
   ...
   `ee doctor --fix --only fm-graph_subsystem-snapshot-missing`, then a follow-up
...
`ee doctor --fix --only fm-graph_subsystem-snapshot-missing` is WIRED. `bd-3boan` (CLI surface for
