...
2. `assert.sh` confirms the marker is present. When
   ...
   `ee doctor --fix --only fm-cass_integration-cass_not_found`, then a follow-up
...
`ee doctor --fix --only fm-cass_integration-cass_not_found` is WIRED. `bd-3boan` (CLI surface for
