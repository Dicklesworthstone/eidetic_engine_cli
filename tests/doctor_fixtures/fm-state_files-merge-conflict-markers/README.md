...
1. `corrupt.sh` builds an isolated corrupt workspace at
   ...
   `.ee/doctor-fixtures/fm-state_files-merge-conflict-markers.json`, plus a baseline
...
2. `assert.sh` confirms the marker is present. When
   ...
   `ee doctor --fix --only fm-state_files-merge-conflict-markers`, then a follow-up
...
`ee doctor --fix --only fm-state_files-merge-conflict-markers` is WIRED. `bd-3boan` (CLI surface for
