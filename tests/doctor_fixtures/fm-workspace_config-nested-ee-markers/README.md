...
1. `corrupt.sh` builds an isolated corrupt workspace at
   ...
   `.ee/doctor-fixtures/fm-workspace_config-nested-ee-markers.json`, plus a baseline
   `.fixture_baseline/before.sha256`.
2. `assert.sh` requires `EE_DOCTOR_FIXTURE_RUN_EE=1` and a binary in
   `EE_DOCTOR_FIXTURE_BINARY` (it exits 2 without them). The damage is a second
   initialized workspace nested inside the target at `nested/child`. The
   independent witness is `ee workspace resolve`, run from inside the child,
   which must report exactly `workspace_nested_markers`. `assert.sh` then runs
   `doctor_fixture_assert_pinned_gap` in `lib.sh` from the same directory:
   `ee doctor` on the outer workspace must report it healthy, and an unscoped
   `ee doctor --fix` must take 0 actions. No undo step runs because nothing is
   written.
...
`manifest.json`). Doctor reports a workspace with a nested store inside it as
healthy, so `ee doctor --fix` dispatches nothing for it. A passing run means
...
nested store is part of the target's own bytes, so the other harnesses see the
same damage without a `condition.sh`.
