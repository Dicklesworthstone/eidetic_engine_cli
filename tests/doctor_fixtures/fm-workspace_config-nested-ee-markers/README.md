...
2. `assert.sh` confirms the marker is present. When
   ...
   `ee doctor --fix --only fm-workspace_config-nested-ee-markers`, then a follow-up
...
`ee doctor --fix --only fm-workspace_config-nested-ee-markers` is WIRED. `bd-3boan` (CLI surface for
