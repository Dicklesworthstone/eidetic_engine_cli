...
2. `assert.sh` confirms the marker is present. When
   ...
   `ee doctor --fix --only fm-state_files-orphaned-pid-write-lock`, then a follow-up
...
`ee doctor --fix --only fm-state_files-orphaned-pid-write-lock` is WIRED. `bd-3boan` (CLI surface for
