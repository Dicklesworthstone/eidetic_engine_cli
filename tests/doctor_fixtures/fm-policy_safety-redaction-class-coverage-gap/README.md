...
1. `corrupt.sh` builds an isolated corrupt workspace at
   ...
   `.ee/doctor-fixtures/fm-policy_safety-redaction-class-coverage-gap.json`, plus a baseline
...
2. `assert.sh` confirms the marker is present. When
   ...
   `ee doctor --fix --only fm-policy_safety-redaction-class-coverage-gap`, then a follow-up
...
`ee doctor --fix --only fm-policy_safety-redaction-class-coverage-gap` is WIRED. `bd-3boan` (CLI surface for
