...
1. `corrupt.sh` builds an isolated corrupt workspace at
   ...
   `.ee/doctor-fixtures/fm-cass_integration-cass_unavailable-contract-mismatch.json`, plus a baseline
...
2. `assert.sh` confirms the marker is present. When
   ...
   `ee doctor --fix --only fm-cass_integration-cass_unavailable-contract-mismatch`, then a follow-up
...
`ee doctor --fix --only fm-cass_integration-cass_unavailable-contract-mismatch` is WIRED. `bd-3boan` (CLI surface for
