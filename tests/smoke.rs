...
#[test]
fn status_skyline_json_stdout_is_stable_machine_data() -> TestResult {
    let output = run_ee(&["status", "--skyline", "--json"])?;
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
        ...
        output.status.success(),
        format!("status --skyline --json should succeed; stderr: {stderr}"),
    )?;
    ensure(
        stderr.is_empty(),
        "stderr must be empty for JSON status skyline".to_string(),
    )?;
    ensure_starts_with(
        &stdout,
        "{\"schema\":\"ee.response.v1\"",
    ...
    connection
        .insert_curation_candidate(
            ...
            &CreateCurationCandidateInput {
                ...
                source_id: Some("fb_00000000000000000000000001".to_string()),
    ...
    connection
        .insert_curation_candidate(
            ...
            &CreateCurationCandidateInput {
                ...
                proposed_trust_class: Some("agent_validated".to_string()),
                source_type: "feedback_event".to_string(),
                source_id: Some("fb_00000000000000000000000002".to_string()),
