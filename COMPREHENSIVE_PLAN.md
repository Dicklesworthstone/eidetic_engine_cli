...
- `asupersync` uses the accepted registry `=0.3.9` profile with `default-features = false` and `tracing-integration`, keeping optional SQLite/Tokio-adjacent adapter surfaces out of the default tree while preserving structured runtime diagnostics.
