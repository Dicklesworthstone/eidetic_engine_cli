...
| `ee tripwire check --revivals [--limit 1..=100] --json` | Evaluate a bounded, deterministic prefix of current workspace-local Revive specs and return those whose predicate passes; the default limit is 25 and capped responses include a higher-limit repair. The explicit command may run allowlisted `ee ... --help` introspection. `ee orient` uses only local read-only predicates and never executes that process. Both paths exclude Gate specs, replace raw targets with domain-separated digests, and perform no automatic result, trust, or tombstone mutation. There is intentionally no cursor because live predicate results can change without a DB generation. |
