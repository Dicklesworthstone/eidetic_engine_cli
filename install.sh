...
detect_platform() {
  ...
  case "${OS}-${ARCH}" in
    linux-x86_64)
      # v0.11.0 ships the gnu artifact for x86_64-linux: the musl static build
      # is unavailable (the Oniguruma C dependency fails the musl static link
      # via cargo-zigbuild). gnu covers every glibc system; Alpine / musl-only
      # hosts should build from source until musl is restored.
