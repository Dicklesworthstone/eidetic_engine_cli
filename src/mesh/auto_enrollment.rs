...
fn stable_bindings_unchanged(
    ...
    selected.iter().all(|candidate| {
        ...
        let mut matched = input
            .existing_peers
            .iter()
            .filter(|peer| peer.enabled && peer.node_key == candidate.node_key);
        matched
            .next()
            .is_some_and(|peer| peer.stable_node_id.as_deref() == Some(stable))
...
mod tests {
    ...
    fn auto_enrollment_stable_id_backfill_is_not_hidden_by_the_key_set_hash() {
        let mut value = input(vec![candidate(VALID_NODE_KEY_ALPHA)]);
        value.existing_peers = vec![existing_auto_peer(
            VALID_NODE_KEY_ALPHA,
            "tailnet-alpha",
            Some("nodekey:self"),
        )];
        ...
        assert!(result.materialization.writes_peer_rows);
        assert_eq!(
            result.materialization.peers_to_upsert[0]
                .stable_node_id
                .as_deref(),
            Some("stable-device")
        );
