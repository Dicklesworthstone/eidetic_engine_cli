...
/// restarted one.
/// Machine surfaces whose output size is already bounded by their own
/// query-level lane and therefore pass through the governor unstamped and
/// untruncated: audit timeline pages at the query (`--limit`/`--cursor`,
/// ADR 0063), and audit show returns exactly one row. Unbounded audit
/// surfaces (diff, verify) are NOT exempt — they carry real truncation
/// points in `OUTPUT_TRUNCATION_REGISTRY` instead, so an output ceiling is
/// honored rather than silently ignored.
pub const QUERY_LANE_EXEMPT_DATA_SCHEMAS: &[&str] = &["ee.audit.timeline.v1", "ee.audit.show.v1"];
