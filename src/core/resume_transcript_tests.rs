...
impl Fixture {
    fn new() -> Self {
        ...
        writer.insert_workspace(&workspace_id, &CreateWorkspaceInput {
            path: workspace.to_string_lossy().into_owned(), name: None,
        }).unwrap();
        Self { _root: root, workspace, database, workspace_id, writer }
    }

    fn options(&self, sessions: usize) -> ResumeOptions<'_> {
        ResumeOptions {
            workspace_path: &self.workspace, database_path: &self.database, sessions,
        }
    }
    ...
    fn session(&self, number: u128, start: Option<&str>, end: Option<&str>) -> String {
        ...
        self.writer.insert_session(&id, &CreateSessionInput {
    ...
    fn evidence(&self, session: &str, number: u128, line: u64, body: &str, parent: Option<&str>) -> StoredEvidenceSpan {
        let id = EvidenceId::from_uuid(uuid::Uuid::from_u128(number)).to_string();
        self.writer.insert_evidence_span(&id, &CreateEvidenceSpanInput {
            workspace_id: self.workspace_id.clone(),
    ...
    fn memory(&self, number: u128) -> String {
        ...
        self.writer.insert_memory(&id, &CreateMemoryInput {
            workspace_id: self.workspace_id.clone(),
            level: "episodic".to_owned(), kind: "note".to_owned(),
            ...
            trust_class: "human_explicit".to_owned(), trust_subclass: None,
            provenance_uri: None, tags: Vec::new(), valid_from: None, valid_to: None,
        }).unwrap();
        id
    }
    ...
    fn state(&self, sessions: usize) -> ResumeState {
        ...
        load(&reader, &self.options(sessions), &self.workspace, reference_time()).unwrap()
    }
}
...
fn transcript_only_resume_preserves_native_identity_and_does_not_retarget_the_store() {
    ...
    let session = fixture.session(1, Some("2020-01-01T09:00:00Z"), Some("2020-01-01T10:00:00Z"));
    let row = fixture.evidence(&session, 2, 7, "Café: migration completed; release validation remains.", None);
    let before = ["memories", "sessions", "evidence_spans", "audit_log", "search_index_jobs"]
        .map(|table| fixture.writer.count_table_rows(table).unwrap());
    let report = build_resume_report(&fixture.options(3)).unwrap();
    assert_eq!(report.episodic_total, 0, "do not relabel evidence as memory rows");
    assert!(report.sessions.is_empty());
    assert!(report.nearby_stores.is_none(), "the addressed store has usable history");
    ...
    assert_eq!(&row.excerpt[item.content_byte_start..item.content_byte_end], item.content);
    ...
    assert!(data["transcriptHistory"]["sessions"][0]["items"][0].get("memoryId").is_none());
    for private in ["PRIVATE-UPSTREAM", "PRIVATE-AGENT", "PRIVATE-SPAN", "/home/private"] {
        assert!(!data.to_string().contains(private));
    }
    let after = ["memories", "sessions", "evidence_spans", "audit_log", "search_index_jobs"]
        .map(|table| fixture.writer.count_table_rows(table).unwrap());
    assert_eq!(before, after);
    assert_eq!(fixture.writer.get_evidence_span(&row.id).unwrap().unwrap(), row);
...
fn source_chronology_not_ingestion_order_controls_recent_session_selection() {
    ...
    assert_eq!(history.sessions[0].activity_at.as_deref(), Some("2020-01-02T10:00:00Z"));
...
fn tail_windows_are_bounded_and_counts_cover_the_entire_admitted_session() {
    ...
    for number in (1..=SESSION_ITEM_CAP + 7).rev() {
        fixture.evidence(&session, number as u128, number as u64, &format!("Window {number}."), None);
    }
    ...
    assert!(session.items.windows(2).all(|pair| pair[0].start_line > pair[1].start_line));
}
...
fn denied_and_tampered_excerpts_do_not_enter_counts_or_session_hints() {
    ...
    fixture.writer.execute_raw(&format!(
        "UPDATE evidence_spans SET search_eligibility = 'denied' WHERE id = '{}'", denied.id
    )).unwrap();
    fixture.writer.execute_raw(&format!(
        "UPDATE evidence_spans SET excerpt = 'TAMPERED-CANARY' WHERE id = '{}'", tampered.id
    )).unwrap();
    let report = build_resume_report(&fixture.options(3)).unwrap();
    assert_eq!(report.transcript_history.session_total, 1);
    assert_eq!(report.transcript_history.evidence_total, 1);
    assert_eq!(report.transcript_history.sessions[0].items[0].evidence_id, safe.id);
    let output = serde_json::to_string(&report).unwrap();
    for hidden in [&denied.id, &tampered.id, &denied_session, "DENIED-CANARY", "TAMPERED-CANARY"] {
...
fn linked_evidence_cannot_resurrect_a_retired_or_closed_seal_memory() {
    ...
    fixture.writer.insert_memory_seal(
        &parent, &crate::models::memory_seal_commitment(b"private history"), "2020-01-01T00:00:00Z"
    ).unwrap();
    assert_eq!(fixture.state(3).transcript_history.evidence_total, 0);
    fixture.writer.mark_memory_seal_revealed(&parent, "2020-01-02T00:00:00Z").unwrap();
    assert_eq!(fixture.state(3).transcript_history.evidence_total, 1);
    fixture.writer.execute_raw(&format!(
...
fn concurrent_revocation_changes_the_next_bundle_not_half_the_current_snapshot() {
    ...
    let state = load_with_boundary(
        &reader, &fixture.options(3), &fixture.workspace, reference_time(), || {
            fixture.writer.execute_raw(&format!(
                "UPDATE evidence_spans SET search_eligibility = 'denied' WHERE id = '{}'", row.id
            )).unwrap();
            Ok(())
        },
    ).unwrap();
    assert_eq!(state.transcript_history.evidence_total, 1);
    assert_eq!(state.transcript_history.sessions[0].items[0].evidence_id, row.id);
    assert_eq!(fixture.state(3).transcript_history.evidence_total, 0);
}
...
fn concurrent_chronology_update_is_not_mixed_with_old_memory_authority() {
    ...
    assert_eq!(state.transcript_history.sessions[0].activity_at.as_deref(), Some("2020-01-01T00:00:00Z"));
    assert_eq!(fixture.state(3).transcript_history.sessions[0].activity_at.as_deref(), Some("2020-01-02T00:00:00Z"));
}
...
fn a_shared_database_never_expands_the_addressed_workspace() {
    ...
    fixture.writer.insert_workspace(&other_id, &CreateWorkspaceInput {
        path: other.to_string_lossy().into_owned(), name: None,
    }).unwrap();
    let reader = DbConnection::open_file_read_only(&fixture.database).unwrap();
    let state = load(&reader, &ResumeOptions {
        workspace_path: &other, database_path: &fixture.database, sessions: 3,
    }, &other, reference_time()).unwrap();
...
fn unicode_prefixes_have_exact_offsets_and_do_not_rewrite_the_source_revision() {
    ...
    assert_eq!(item.entity_revision, row.pack_entity_revision());
    assert_eq!(fixture.writer.get_evidence_span(&row.id).unwrap().unwrap().excerpt, body);
}
...
fn corrupt_chronology_fails_closed_without_leaking_storage_values_or_leaving_a_snapshot() {
    ...
    let reader = DbConnection::open_file_read_only(&fixture.database).unwrap();
    let error = load(&reader, &fixture.options(3), &fixture.workspace, reference_time()).err().unwrap();
    let message = error.to_string();
    assert!(!message.contains("PRIVATE-INVALID-TIME"));
    assert!(!message.contains(&session));
    assert!(matches!(error, DomainError::Storage { .. }));
    reader.begin_read_snapshot().expect("failed resume released its snapshot");
...
fn header_pagination_and_equal_time_ties_do_not_drop_later_sessions() {
    ...
    let later = SessionId::from_uuid(uuid::Uuid::from_u128((RESUME_STORAGE_PAGE_SIZE + 1) as u128)).to_string();
    ...
    let history = fixture.state(1).transcript_history;
    assert_eq!(history.session_total, 2, "unadmitted headers are not resumable sessions");
