...
fn assert_late_change_refused(table: &str, sql: &str) -> TestResult {
    ...
    let error = restore_backup_to_side_path_with_recovery_hooks(
        ...
        |path| {
            ...
            let store = path.parent().ok_or_else(|| work_history_error("missing staged store"))?;
            ...
            let before = tables.iter().map(|t| db.count_table_rows(t))
                .collect::<Result<Vec<_>, _>>().map_err(work_history_error)?;
            ...
            let after = tables.iter().map(|t| db.count_table_rows(t))
                .collect::<Result<Vec<_>, _>>().map_err(work_history_error)?;
            assert_eq!(before, after);
            db.close().map_err(work_history_error)
        },
    ).err().ok_or("published authority modified after index rebuild")?;
    ...
    assert!(staged.is_file(), "retain failed staged state for inspection");
    let source = DbConnection::open_file(&fixture.database).map_err(|e| e.to_string())?;
...
fn publication_fence_rejects_quarantine_release_after_index_rebuild() -> TestResult {
    assert_late_change_refused("trust_quarantine", "UPDATE trust_quarantine SET status = 'released'")
}

#[test]
fn publication_fence_rejects_fabricated_reveal_after_index_rebuild() -> TestResult {
    assert_late_change_refused("memory_seals", "UPDATE memory_seals SET revealed_at = '2026-09-02T00:00:00Z', reveal_verified = 1")
}

#[test]
fn publication_fence_rejects_fabricated_verification_after_index_rebuild() -> TestResult {
    assert_late_change_refused("certificates", "UPDATE certificates SET status = 'valid', verified_at = '2026-09-02T00:00:00Z'")
}
