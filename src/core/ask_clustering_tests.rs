...
fn all_pairs_reference(spans: &[AskSpan]) -> Vec<AskSpan> {
    ...
    for &seed in &order {
        ...
        for &other in &order {
            if assigned[other]
                || has_negation(&spans[seed].text) != has_negation(&spans[other].text)
            {
                continue;
            }
            if jaccard_similarity(&term_sets[seed], &term_sets[other])
...
fn agrees_bit_for_bit_with_all_pairs_on_seeded_corpora() {
    ...
    for round in 0..64 {
        ...
        assert_eq!(
            signature(&cluster_spans(&input)),
            signature(&all_pairs_reference(&input)),
            "corpus {round}"
        );
        input.reverse();
        assert_eq!(
...
fn opposition_and_same_memory_repetition_do_not_inflate_corroboration() {
    ...
    assert_eq!(
        ...
        (0.7_f32 * (1.0 + 0.1 * 2.0_f32.ln())).to_bits()
    );
}
...
fn ten_thousand_disjoint_spans_need_zero_pairwise_similarity_checks() {
    let input: Vec<_> = (0..10_000)
        .map(|index| span(index, index, &format!("unique_term_{index}"), 0.5))
        .collect();
    ...
    assert_eq!(
        actual.len(),
        input.len(),
        "no evidence was truncated to get the work reduction"
    );
    ...
    assert!(
        actual
            .iter()
            .all(|span| span.score.to_bits() == 0.5_f32.to_bits())
    );
}
