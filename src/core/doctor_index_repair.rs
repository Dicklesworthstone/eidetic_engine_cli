...
impl Inventory {
    fn copy_budget(&self, root: &Path) -> Result<(u64, u64), DoctorRuntimeError> {
        let overflow = || {
            repair_error(
                "estimate doctor repair copy",
                "size or entry count overflow",
            )
        };
        let mut bytes = 0_u64;
        ...
        for relative in &self.files {
            ...
            let path_entries = path
                ...
                .ok_or_else(&overflow)?;
            entries = entries
                .checked_add(u64::try_from(path_entries).map_err(|_| overflow())?)
                .ok_or_else(&overflow)?;
        }
        Ok((bytes, entries))
    }
...
pub(super) fn rebuild(
    ...
    crate::core::run_cli_with_cx(Duration::from_secs(300), |cx| async move {
        ...
        let bytes = new_bytes.checked_add(old_bytes).ok_or_else(|| {
            repair_error("estimate doctor index publication", "byte count overflow")
        })?;
        let entries = new_entries.checked_add(old_entries).ok_or_else(|| {
            repair_error("estimate doctor index publication", "entry count overflow")
        })?;
...
mod tests {
    ...
    fn empty_corpus_rebuild_preserves_tombstones_and_remains_undoable() {
        ...
        let metadata: serde_json::Value =
            serde_json::from_slice(&fs::read(index.join(MANIFEST)).expect("empty metadata"))
                .expect("metadata JSON");
        ...
        let summary = ctx.finish(RunStatus::CompletedOk).expect("finish");
        let undo =
            replay_undo_for_workspace(&workspace, &summary.run_id).expect("undo empty generation");
        assert_eq!(undo.status, RunStatus::Undone, "{:?}", undo.first_error);
        assert!(!index.exists());
        assert_eq!(
            fs::read(database).expect("tombstone preserved after undo"),
            before
        );
    }
    ...
    fn undo_refuses_a_newer_index_manifest_without_touching_tiers() {
        ...
        let mut metadata: serde_json::Value =
            serde_json::from_slice(&fs::read(&marker).expect("original repaired manifest"))
                .expect("metadata JSON");
        ...
        let undo =
            replay_undo_for_workspace(&workspace, &summary.run_id).expect("bounded undo refusal");
        ...
        assert_eq!(
            fs::read(index.join("vector.fast.idx")).expect("tier preserved"),
            tier_before
        );
    }
