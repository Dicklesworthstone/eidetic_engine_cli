...
fn real_import_restores_scoped_recall_without_claiming_unassigned_memories() {
    ...
    let records = [record(1, Some("release-a")), record(2, Some("release-a")),
        record(3, Some("release-b")), record(4, None)];
    ...
    let workspace = db.get_memory(&records[0].memory_id).unwrap().unwrap().workspace_id;
    let related = db.list_recent_workflow_memories(&workspace, "release-a", &records[0].memory_id, 10).unwrap();
    assert_eq!(related.len(), 1);
    assert_eq!(related[0].id, records[1].memory_id);
    assert!(db.list_recent_workflow_memories(&workspace, "release-b", &records[2].memory_id, 10).unwrap().is_empty());
    db.close().unwrap();
}
...
fn invalid_or_secret_workflow_ids_fail_before_destination_creation() {
    ...
    for (bad, code) in [
        ...
        (json!(format!("sk-proj-{}", "a".repeat(44))), "memory_workflow_id_contains_secret"),
        (json!("api_key=WORKFLOW_PRIVATE_CANARY"), "memory_workflow_id_contains_secret"),
    ] {
        let mut rows = baseline.lines().map(|line| serde_json::from_str::<JsonValue>(line).unwrap()).collect::<Vec<_>>();
        ...
        for dry_run in [false, true] {
            ...
            assert!(report.issues.iter().any(|issue| issue.code == code), "{:?}", report.issues);
...
fn reimport_cannot_add_remove_or_reassign_workflows_or_publish_jobs() {
    for original_workflow in [Some("release-a"), None] {
        ...
        let opts = options(root.path(), &source(std::slice::from_ref(&original), RedactionLevel::None));
        ...
        for workflow in [original_workflow, Some("release-b"), None] {
            ...
            let conflict = report.issues.iter().any(|issue| issue.code == "reimport_divergent_existing_row" && issue.message.contains("workflow_id"));
            assert_eq!(conflict, workflow != original_workflow, "{:?}", report.issues);
            let db = DbConnection::open_file(database_path(&opts)).unwrap();
...
fn workflow_redaction_is_non_aliasing_and_stable_across_recovery_generations() {
    ...
    for level in [RedactionLevel::None, RedactionLevel::Minimal, RedactionLevel::Standard,
        ...
        let projected = redact_memory_record(record(1, Some(first)), level).workflow_id.unwrap();
        let other = redact_memory_record(record(2, Some(second)), level).workflow_id.unwrap();
        assert_ne!(projected, other);
        assert_eq!(redact_recovery_identity(&projected, level), projected);
        assert_eq!(redact_memory_record(record(1, Some(&projected)), level).workflow_id.as_deref(), Some(projected.as_str()));
    ...
    assert_ne!(redact_recovery_identity("key_secret", RedactionLevel::Full), "key_secret");
}

#[test]
fn redacted_workflows_import_and_remain_usable_for_scoped_recall() {
    let original = "api_key=WORKFLOW_PRIVATE_CANARY";
    for level in [RedactionLevel::Minimal, RedactionLevel::Standard, RedactionLevel::Strict,
        ...
        let related = db.list_recent_workflow_memories(&stored.workspace_id, &expected, "not-this-memory", 10).unwrap();
        assert_eq!(related.len(), 1);
