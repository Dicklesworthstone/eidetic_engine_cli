...
async fn build_generation(cx: &asupersync::Cx, path: &Path, generation: u64) -> TestResult {
    ...
    build_lexical_tier(cx, path, &documents).await.map_err(|error| error.to_string())?;
...
fn inventory(root: &Path) -> Result<BTreeMap<PathBuf, Option<Vec<u8>>>, String> {
    ...
    while let Some(path) = pending.pop() {
        ...
        if metadata.is_dir() {
            ...
            result.insert(relative, Some(std::fs::read(&path).map_err(|error| error.to_string())?));
        } else {
...
fn rejected_directory(index: &Path) -> Result<PathBuf, String> {
    let prefix = format!(".{}{}", index_base_name(index).map_err(|error| error.to_string())?, INDEX_REJECTED_PREFIX);
...
fn assert_retired_manifest(path: &Path, expected: &[u8]) -> TestResult {
    ...
    for entry in std::fs::read_dir(path).map_err(|error| error.to_string())? {
        let entry = entry.map_err(|error| error.to_string())?;
        if entry.file_name().to_string_lossy().starts_with(super::RETIRED_PREFIX) {
...
fn failed_rollback_preparation_cannot_admit_the_rejected_live_index() -> TestResult {
    ...
    crate::core::run_cli_with_cx(Duration::from_secs(60), |cx| async move {
        ...
        let manifest = std::fs::read(index.join(INDEX_METADATA_FILE)).map_err(|error| error.to_string())?;
        ...
        let error = rollback_published_index(&index, &parent.join("absent-stage"), Some(&broken_retained))
            .expect_err("invalid retained directory must fail rollback");
        assert!(error.to_string().contains("not a directory"));
        assert!(index.is_dir(), "failure must exercise a still-live rejected directory");
        assert_retired_manifest(&index, &manifest)?;
        let before = inventory(parent)?;
        let lease = IndexGenerationLease::read(&cx, &index).await.map_err(|error| error.to_string())?;
        assert_eq!(lease.index_for_snapshot(&cx, &index, 8).map_err(|error| error.to_string())?, accepted);
        assert_eq!(inventory(parent)?, before, "reader recovery must remain read-only");
        Ok(())
    }).map_err(|error| error.to_string())?
}
...
fn failed_commit_retires_only_the_rejected_generation_manifest() -> TestResult {
    ...
    crate::core::run_cli_with_cx(Duration::from_secs(60), |cx| async move {
        ...
        let accepted_manifest = std::fs::read(index.join(INDEX_METADATA_FILE)).map_err(|error| error.to_string())?;
        let staging = create_publish_staging_dir(&index).map_err(|error| error.to_string())?;
        build_generation(&cx, &staging, 8).await?;
        let rejected_manifest = std::fs::read(staging.join(INDEX_METADATA_FILE)).map_err(|error| error.to_string())?;
        let error = publish_staged_index_with_commit(&index, &staging, || {
            Err(IndexRebuildError::Index("controlled bookkeeping failure".into()))
        ...
        assert_eq!(std::fs::read(index.join(INDEX_METADATA_FILE)).map_err(|error| error.to_string())?, accepted_manifest);
        assert_retired_manifest(&rejected_directory(&index)?, &rejected_manifest)?;
        let lease = IndexGenerationLease::read(&cx, &index).await.map_err(|error| error.to_string())?;
        assert_eq!(lease.index_for_snapshot(&cx, &index, 8).map_err(|error| error.to_string())?, index);
        Ok(())
    }).map_err(|error| error.to_string())?
}
...
fn rejected_first_publication_cannot_be_reintroduced_as_retained() -> TestResult {
    ...
    crate::core::run_cli_with_cx(Duration::from_secs(60), |cx| async move {
        let staging = create_publish_staging_dir(&index).map_err(|error| error.to_string())?;
        build_generation(&cx, &staging, 8).await?;
        let manifest = std::fs::read(staging.join(INDEX_METADATA_FILE)).map_err(|error| error.to_string())?;
        assert!(publish_staged_index_with_commit(&index, &staging, || {
            Err(IndexRebuildError::Index("controlled first publication failure".into()))
        }).is_err());
        assert!(!index.exists());
        ...
        drop(lease);
        assert_eq!(recover_interrupted_publish(&index).map_err(|error| error.to_string())?, IndexPublishRecoveryAction::NoRecoverableGeneration);
        assert!(!index.exists());
        assert_eq!(inventory(index_parent(&index))?, before);
        Ok(())
    }).map_err(|error| error.to_string())?
...
fn manifest_retirement_failure_does_not_skip_restoration_of_the_accepted_index() -> TestResult {
    ...
    crate::core::run_cli_with_cx(Duration::from_secs(60), |cx| async move {
        ...
        build_generation(&cx, &index, 8).await?;
        std::fs::rename(index.join(INDEX_METADATA_FILE), index.join("injected-manifest.saved"))
            .map_err(|error| error.to_string())?;
        ...
        assert_eq!(validated_index_generation(&index)?, 7);
        assert!(rejected_directory(&index)?.join("injected-manifest.saved").is_file());
        Ok(())
    }).map_err(|error| error.to_string())?
}
...
fn unsafe_retained_path_cannot_leave_rejected_live_manifest_admissible() -> TestResult {
    ...
    crate::core::run_cli_with_cx(Duration::from_secs(60), |cx| async move {
        ...
        assert!(rollback_published_index(&index, &parent.join("absent-stage"), Some(&link)).is_err());
        assert_retired_manifest(&index, &manifest)?;
        assert_eq!(validated_index_generation(&accepted)?, 7);
        assert!(std::fs::symlink_metadata(&link).map_err(|error| error.to_string())?.is_symlink());
        let lease = IndexGenerationLease::read(&cx, &index).await.map_err(|error| error.to_string())?;
        assert_eq!(lease.index_for_snapshot(&cx, &index, 8).map_err(|error| error.to_string())?, accepted);
        Ok(())
    }).map_err(|error| error.to_string())?
...
fn rejected_live_generation_with_no_accepted_fallback_fails_closed() -> TestResult {
    ...
    crate::core::run_cli_with_cx(Duration::from_secs(60), |cx| async move {
        ...
        assert!(rollback_published_index(&index, &parent.join("absent-stage"), Some(&invalid)).is_err());
        let before = inventory(parent)?;
        let lease = IndexGenerationLease::read(&cx, &index).await.map_err(|error| error.to_string())?;
        assert!(lease.index_for_snapshot(&cx, &index, 8).is_err());
