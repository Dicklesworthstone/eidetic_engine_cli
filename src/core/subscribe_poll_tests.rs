...
impl Fixture {
    ...
    fn audit(&self, workspace: Option<&str>, target: Option<&str>, kind: Option<&str>, action: &str) -> u64 {
        ...
        let rows = self.db.query("SELECT MAX(rowid) FROM audit_log", &[]).expect("cursor");
        rows[0].get(0).and_then(|value| value.as_i64()).expect("rowid") as u64
    }
...
fn subscription_is_workspace_bound_even_in_a_shared_database() {
    ...
    let local_cursor = f.audit(Some(&f.own), Some(&local), Some("memory"), audit_actions::MEMORY_CREATE);
    f.audit(Some(&f.other), Some(&foreign), Some("memory"), audit_actions::MEMORY_CREATE);
    // Neither side of a mismatched audit/source join grants workspace authority.
    f.audit(Some(&f.own), Some(&foreign), Some("memory"), audit_actions::MEMORY_CREATE);
    f.audit(Some(&f.other), Some(&local), Some("memory"), audit_actions::MEMORY_CREATE);
    f.audit(Some(&f.own), Some(&local), Some("rule"), audit_actions::TRUST_CLASS_TRANSITION);
    ...
    assert_eq!(report.deltas[0].workspace_id.as_deref(), Some(f.own.as_str()));
    ...
    assert!(narrowed.deltas.is_empty(), "filters cannot broaden workspace scope");
}
...
fn absent_memory_does_not_hide_an_owned_tombstone_invalidation() {
    ...
    f.audit(Some(&f.own), Some(&missing), Some("memory"), audit_actions::MEMORY_TOMBSTONE);
    let report = f.poll(0, 100, SubscribeFilter::default());
    assert_eq!(report.delta_count, 1);
...
fn filtered_empty_pages_keep_the_lookahead_and_advance_through_nonmemory_tail() {
    ...
    let mut ids = Vec::new();
    for number in 1..=5 {
        let id = f.memory(&f.own, number, if number <= 2 { "other" } else { "release" });
        cursors.push(f.audit(Some(&f.own), Some(&id), Some("memory"), audit_actions::MEMORY_CREATE));
        ids.push(id);
    }
    ...
    assert_eq!(second.deltas.iter().map(|d| &d.memory_id).collect::<Vec<_>>(), vec![&ids[2], &ids[3]]);
...
fn subscription_snapshot_pins_watermark_rows_and_tags_across_real_writes() {
    ...
    let before = f.audit(Some(&f.own), Some(&id), Some("memory"), audit_actions::MEMORY_CREATE);
    let reader = DbConnection::open(DatabaseConfig::read_only_file(f.database.clone())).expect("reader");
    let snapshot = SubscriptionSnapshot::begin(&reader).expect("snapshot");
    assert_eq!(snapshot.high_watermark(&f.own).expect("pin"), before);
    f.db.execute("UPDATE memories SET kind = 'decision' WHERE id = ?1", &[SqlValue::Text(id.clone())]).expect("change kind");
    f.db.execute("UPDATE memory_tags SET tag = 'other' WHERE memory_id = ?1", &[SqlValue::Text(id.clone())]).expect("change tag");
    f.audit(Some(&f.own), Some(&id), Some("memory"), audit_actions::MEMORY_UPDATE);
    let page = snapshot.page(&f.own, 0, 100, &SubscribeFilter::default(), None).expect("pinned page");
...
fn dropped_subscription_snapshot_releases_only_its_own_transaction() {
    ...
    outer.finish().expect("nested failure did not roll back outer");
    {
        let _early_return = SubscriptionSnapshot::begin(&f.db).expect("snapshot");
    }
    f.db.begin_read_snapshot().expect("released on drop");
    f.db.rollback_read_snapshot().expect("release test snapshot");
}
...
fn polling_never_creates_or_migrates_a_store() {
    ...
    let options = SubscribePollOptions { database_path: Some(&blank), ..options };
    assert!(poll_memory_deltas(&options).is_err());
...
fn stale_unsigned_cursor_has_explicit_resynchronization_without_overflow() {
    let f = Fixture::new();
    let id = f.memory(&f.own, 1, "release");
    let cursor = f.audit(Some(&f.own), Some(&id), Some("memory"), audit_actions::MEMORY_CREATE);
