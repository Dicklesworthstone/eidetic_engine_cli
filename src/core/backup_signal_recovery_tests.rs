...
type SignalState = (
    Vec<StoredLearningObservation>,
    Vec<StoredFeedbackQuarantine>,
    Vec<StoredOutcomeEvidence>,
);
...
fn quarantine_id(n: usize) -> String {
    format!("fq_{n:026}")
}
...
fn read_state(db: &DbConnection) -> Result<SignalState, String> {
    let ws = workspace_id();
    Ok((
        db.list_learning_observations(&ws, None)
            .map_err(|e| e.to_string())?,
        db.list_feedback_quarantine(&ws, None)
            .map_err(|e| e.to_string())?,
        db.list_outcome_evidence_for_recovery(&ws)
            .map_err(|e| e.to_string())?,
    ))
}
...
fn fixture(level: RedactionLevel) -> Result<Fixture, String> {
    ...
    db.insert_learning_observation_for_recovery(&StoredLearningObservation {
        id: "lobs_recovery_signal".to_owned(),
    ...
    for n in 0..2 {
        let mut row = StoredFeedbackQuarantine {
            ...
            proposed_event_id: Some(format!("fb_{:026}", 100 + n)),
    ...
    for (n, source) in [
        OutcomeEvidenceSource::VerifierSuccess,
        OutcomeEvidenceSource::ExplicitHuman,
    ]
    .into_iter()
    .enumerate()
    {
        let mut row = StoredOutcomeEvidence {
            ...
            evidence_ref: format!("untracked-observation-{n}"),
...
fn assert_refused(table: &str, mutate: impl FnOnce(&DbConnection) -> TestResult) -> TestResult {
    ...
    let error = restore_backup_to_side_path_with_verification_hook(&fixture.options, |path| {
        ...
        assert_ne!(
            read_state(&db).map_err(work_history_error)?,
            before,
            "mutation must change actual readable rows"
        );
        ...
        assert_eq!(
            counts, after_counts,
            "a count-only fence must not detect this mutation"
        );
        db.close().map_err(work_history_error)
    })
    .err()
    .ok_or("published altered learning evidence")?;
    assert!(
        error
            .message()
    ...
    assert_eq!(
        source
            .count_table_rows("feedback_events")
            .map_err(|e| e.to_string())?,
        0
    );
    source.close().map_err(|e| e.to_string())?;
...
fn rewrite_quarantine_payload(db: &DbConnection, invalid_source: bool) -> TestResult {
    ...
    execute(
        db,
        &format!(
            "UPDATE feedback_quarantine SET weight = {}, raw_event_hash = '{hash}' WHERE id = '{id}'",
            row.weight
        ),
    )?;
    let actual = db
        ...
        .into_iter()
        .find(|row| row.id == id)
        .ok_or("missing changed quarantine")?;
    assert_eq!(
        quarantine_payload_hash(&actual)
            .map_err(|e| e.message())?
            .as_deref(),
        Some(actual.raw_event_hash.as_str())
    );
    Ok(())
}
...
fn rewrite_outcome(
    db: &DbConnection,
    change: impl FnOnce(&mut StoredOutcomeEvidence),
) -> TestResult {
    let mut row = db
        .list_outcome_evidence_for_recovery(&workspace_id())
        .map_err(|e| e.to_string())?
        .into_iter()
        .find(|row| row.source == OutcomeEvidenceSource::VerifierSuccess)
        .ok_or("missing verifier result")?;
    ...
    execute(
        db,
        &format!(
            "UPDATE outcome_evidence_rows SET source_kind = '{}', evidence_family = '{}', base_weight_milli = {}, evidence_ref = '{}', observed_at = '{}', provenance_hash = '{}' WHERE source_kind = '{original_source}' AND evidence_ref = '{original_ref}'",
            row.source.as_str(),
            row.evidence_family,
            row.base_weight_milli,
            row.evidence_ref,
            row.observed_at,
            row.provenance_hash
        ),
    ...
    let actual = db
        ...
        .map_err(|e| e.to_string())?
        .into_iter()
        .find(|actual| actual.evidence_ref == row.evidence_ref && actual.source == row.source)
        .ok_or("missing changed outcome")?;
...
fn signal_fence_rejects_observation_direction_change() -> TestResult {
    assert_refused("learning_observations", |db| {
        execute(db, "UPDATE learning_observations SET signal = 'harmful'")
    })
}

#[test]
fn signal_fence_rejects_fabricated_feedback_review() -> TestResult {
    assert_refused("feedback_quarantine", |db| {
        execute(
            db,
            "UPDATE feedback_quarantine SET status = 'rejected', reviewed_at = '2026-09-02T00:00:00Z', reviewed_by = 'PRIVATE_SENTINEL'",
        )
    })
}
...
fn signal_fence_rejects_self_consistent_feedback_weight_change() -> TestResult {
    assert_refused("feedback_quarantine", |db| {
        rewrite_quarantine_payload(db, false)
    })
}
...
fn signal_fence_rejects_repairing_invalid_source_into_releasable_feedback() -> TestResult {
    assert_refused("feedback_quarantine", |db| {
        rewrite_quarantine_payload(db, true)
    })
}

#[test]
fn signal_fence_rejects_self_consistent_escalation_to_human_authority() -> TestResult {
    assert_refused("outcome_evidence_rows", |db| {
        rewrite_outcome(db, |row| {
...
fn signal_fence_rejects_self_consistent_evidence_reattribution() -> TestResult {
    assert_refused("outcome_evidence_rows", |db| {
        rewrite_outcome(db, |row| row.evidence_ref = "PRIVATE_SENTINEL".to_owned())
    })
}
...
fn signal_fence_rejects_self_consistent_outcome_chronology_change() -> TestResult {
    assert_refused("outcome_evidence_rows", |db| {
        rewrite_outcome(db, |row| {
            row.observed_at = "2026-08-31T00:00:00Z".to_owned()
        })
    })
}
...
fn learning_signals_survive_rebackup_without_reapplying_or_validating_feedback() -> TestResult {
    for level in [RedactionLevel::None, RedactionLevel::Full] {
        ...
        let db =
            DbConnection::open_file(&first.restored_database_path).map_err(|e| e.to_string())?;
        let expected = read_state(&db)?;
        assert_eq!(
            (expected.0.len(), expected.1.len(), expected.2.len()),
            (1, 2, 2)
        );
        ...
        let second = restore_backup_to_side_path(&BackupRestoreOptions {
            ...
            backup_path: PathBuf::from(backup.backup_path),
            side_path: fixture.root.path().join("second"),
            restore_graph_cache: false,
            dry_run: false,
        })
        .map_err(|e| e.message())?;
        let db =
            DbConnection::open_file(&second.restored_database_path).map_err(|e| e.to_string())?;
        assert_eq!(
            read_state(&db)?,
            expected,
            "observation identity, invalid source hashes, authority and chronology must remain exact"
        );
        assert_eq!(
            db.count_table_rows("feedback_events")
                .map_err(|e| e.to_string())?,
