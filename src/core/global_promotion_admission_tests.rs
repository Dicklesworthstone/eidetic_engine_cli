...
use crate::core::global_promotion::{
    BackflowOptions, BackflowSignal, DemoteGlobalOptions, backflow_global_feedback,
    demote_global, promote_global,
};
...
impl Fixture {
    fn new() -> Self {
        ...
        .unwrap();
        db.insert_memory(MEMORY, &memory_input(WORKSPACE, BODY)).unwrap();
        Self {
            _root: root,
    ...
    fn update(&self, fields: &str) {
        self.db.execute_raw(&format!("UPDATE memories SET {fields} WHERE id = '{MEMORY}'")).unwrap();
    }

    fn seal(&self) {
        self.db.insert_memory_seal(
            MEMORY,
...
fn refused_sources_never_initialize_or_inspect_the_destination() {
    for (update, code) in [
        ("trust_class = 'agent_assertion'", "global_promotion_evidence_gate"),
        ("tombstoned_at = '2001-01-01T00:00:00Z'", "global_promotion_tombstoned"),
        ("superseded_at = '2001-01-01T00:00:00Z'", "global_promotion_superseded"),
        ("valid_to = '2001-01-01T00:00:00Z'", "global_promotion_expired"),
        ("valid_from = '2098-01-01T00:00:00Z'", "global_promotion_not_yet_valid"),
    ] {
        let fixture = Fixture::new();
    ...
    assert_eq!(promote_global(&options).unwrap().plan.data_json()["detail"]["code"], "global_lane_unavailable");
    assert_eq!(disk(&fixture.root), before);
}
...
fn author_windows_use_inclusive_instants_and_validate_both_bounds() {
    ...
    assert_eq!(lifecycle_refusal(Some("2029-12-31T19:00:00-05:00"), Some("2030-01-01T01:00:00+01:00"), None, reference).unwrap(), None);
    assert_eq!(lifecycle_refusal(Some("2030-01-01T00:00:00.000000001Z"), None, None, reference).unwrap(), Some(PromotionRefusal::NotYetValid));
    assert_eq!(lifecycle_refusal(None, Some("2029-12-31T23:59:59.999999999Z"), None, reference).unwrap(), Some(PromotionRefusal::Expired));
    assert_eq!(lifecycle_refusal(None, None, Some(TO), reference).unwrap(), Some(PromotionRefusal::Superseded));
...
fn seals_control_promotion_independently_of_body_spelling_and_explicit_reveal() {
    ...
    assert_eq!(closed.plan.data_json()["detail"]["code"], "global_promotion_sealed");
    assert!(!closed.data_json().to_string().contains(BODY));
    assert_eq!(disk(&fixture.root), before);
    fixture.db.mark_memory_seal_revealed(MEMORY, FROM).unwrap();
    fixture.update(&format!("content = '{}'", crate::models::MEMORY_SEAL_PLACEHOLDER_CONTENT));
    assert!(promote_global(&fixture.options(true)).unwrap().plan.allowed());
    fixture.update("superseded_at = '2001-01-01T00:00:00Z'");
    assert_eq!(promote_global(&fixture.options(false)).unwrap().plan.data_json()["detail"]["code"], "global_promotion_superseded");
    assert!(!fixture.global.root.exists());
}
...
fn body_and_seal_authority_observe_one_snapshot_during_a_concurrent_write() {
    ...
    assert_eq!(next.data_json()["detail"]["code"], "global_promotion_sealed");
    assert!(!fixture.global.root.exists());
}
...
fn successful_publication_preserves_author_expiry_and_source_body() {
    ...
    let copy = db.get_memory(promoted.global_memory_id.as_deref().unwrap()).unwrap().unwrap();
...
fn duplicate_preview_is_read_only_and_does_not_accept_hidden_or_short_lived_twins() {
    let fixture = Fixture::new();
    let (global, workspace) = crate::core::global_store::open_or_create_global_store(&fixture.global).unwrap();
    global.insert_memory(OTHER, &memory_input(&workspace, BODY)).unwrap();
    let before = disk(&fixture.root);
    let report = promote_global(&fixture.options(true)).unwrap();
    ...
    for update in [
        ...
        global.execute_raw(&format!("UPDATE memories SET {update} WHERE id = '{OTHER}'")).unwrap();
        assert!(promote_global(&fixture.options(true)).unwrap().global_memory_id.is_none());
    }
    global.execute_raw(&format!("UPDATE memories SET kind = 'rule' WHERE id = '{OTHER}'")).unwrap();
    global.insert_memory_seal(OTHER, &crate::models::memory_seal_commitment(BODY.as_bytes()), FROM).unwrap();
    assert!(promote_global(&fixture.options(true)).unwrap().global_memory_id.is_none());
}
...
fn wrong_workspace_targets_and_nonfinite_feedback_fail_before_writes() {
    let fixture = Fixture::new();
    let (global, _) = crate::core::global_store::open_or_create_global_store(&fixture.global).unwrap();
    global.insert_workspace(WORKSPACE, &CreateWorkspaceInput {
    ...
    for dry_run in [true, false] {
        assert!(demote_global(&DemoteGlobalOptions {
        ...
        for weight in [0.03, f32::NAN, f32::INFINITY, f32::NEG_INFINITY] {
            assert!(backflow_global_feedback(&BackflowOptions {
                workspace_database_path: &fixture.database,
                global_memory_id: OTHER,
                global_paths: &fixture.global,
                signal: BackflowSignal::Helpful,
    ...
    assert_eq!(global.count_table_rows("feedback_events").unwrap(), feedback_events);
}
