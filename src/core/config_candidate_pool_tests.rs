...
fn candidate_pool_set_round_trips_and_dry_run_does_not_write() {
    ...
    assert_eq!(
        ConfigFile::parse(&dry.planned_toml)
            .unwrap()
            .pack
            .candidate_pool,
        Some(24)
...
fn candidate_pool_updates_preserve_other_settings_and_invalid_writes_are_atomic() {
    ...
    assert_eq!(
        get_config(&opts, SEARCH_DEFAULT_SPEED_KEY).unwrap().value,
        "fast"
    );
    for raw in [
        "0",
        ...
        for dry_run in [false, true] {
            ...
            assert!(
                matches!(error, ConfigSurfaceError::InvalidValue { .. }),
                "{raw}: {error}"
            );
            assert_eq!(
...
fn candidate_pool_toml_and_setter_accept_the_same_positive_u32_range() {
    ...
    for value in [1, 100, u32::MAX] {
        ...
        assert_eq!(
            ConfigFile::parse(&report.planned_toml)
                .unwrap()
                .pack
                .candidate_pool,
            parsed.pack.candidate_pool
        );
    }
    for raw in [
        "0",
        ...
        assert!(
            error.to_string().contains(PACK_CANDIDATE_POOL_KEY),
            "{error}"
        );
    }
    assert_eq!(
        ConfigFile::parse("[pack]\n").unwrap().pack.candidate_pool,
        None
    );
}
...
fn candidate_pool_default_user_project_and_explicit_precedence() {
    ...
    assert_eq!(
        merged.source(PACK_CANDIDATE_POOL_KEY),
    ...
    assert_eq!(
        resolve_pack_candidate_pool(&workspace, None).unwrap(),
        Some(7)
    );
    // 100 is an explicit override, never a sentinel for an omitted flag.
    assert_eq!(
        resolve_pack_candidate_pool(&workspace, Some(100)).unwrap(),
        Some(100)
    );
    assert_eq!(
        resolve_pack_candidate_pool(&workspace, Some(11)).unwrap(),
        Some(11)
    );
}
