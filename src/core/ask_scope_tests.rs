...
fn seed(
    ...
    db.insert_memory(
        ...
        &CreateMemoryInput {
            ...
            content: format!("Run cargo fmt before release. Source {number}."),
...
fn member(db: &DbConnection, workspace: &str, id: &str, name: &str, state: &str) {
    db.insert_team_member(&InsertTeamMemberInput {
        ...
        is_self: false,
        origin_node_id: format!("node_{name}"),
...
fn self_scope_requires_identity_and_excludes_other_agents_from_hints() {
    ...
    assert!(
        load(&db, &workspace, MemoryScope::SelfOnly, None)
            .candidates
            .is_empty()
    );
}
...
fn verified_scope_uses_native_trust_classes_not_asserted_agent_names() {
    ...
    assert_eq!(
        ids(&load(&db, &workspace, MemoryScope::Verified, None)),
        [human.as_str(), validated.as_str()]
    );
}
...
fn global_scope_accepts_only_explicit_tags_without_workspace_expansion() {
    ...
    db.insert_workspace(
        &foreign,
        &CreateWorkspaceInput {
            path: "/ask-other-workspace".to_owned(),
            name: None,
        },
    )
    .unwrap();
    ...
    assert_eq!(
        ids(&load(&db, &workspace, MemoryScope::Global, None)),
        [global.as_str(), rule.as_str()]
    );
    assert_eq!(
        load(&db, &workspace, MemoryScope::Workspace, None)
            .candidates
            .len(),
        3
    );
    assert_eq!(
        load(&db, &workspace, MemoryScope::Swarm, None)
            .candidates
            .len(),
        3
    );
...
fn team_scope_uses_active_workspace_roster_and_native_node_attribution() {
    ...
    member(
        &db,
        &workspace,
        "mbr_00000000000000000000000000000001",
        "Bob",
        "active",
    );
    member(
        &db,
        &workspace,
        "mbr_00000000000000000000000000000002",
        "Carol",
        "removed",
    );
    ...
    db.insert_workspace(
        &foreign,
        &CreateWorkspaceInput {
            path: "/ask-foreign-roster".to_owned(),
            name: None,
        },
    )
    .unwrap();
    member(
        &db,
        &foreign,
        "mbr_00000000000000000000000000000003",
        "Mallory",
        "active",
    );
    assert_eq!(
        ids(&load(&db, &workspace, MemoryScope::Team, Some("Alice"))),
        [own.as_str(), active.as_str(), node.as_str()]
    );
}
...
fn tag_changes_during_read_are_visible_only_to_the_next_snapshot() {
    ...
    let corpus = load_corpus_with_scope_boundary(
        ...
        || {
            writer
                .with_transaction(|| writer.remove_memory_tags(&id, &["global".to_owned()]))
                .unwrap();
            Ok(())
        },
    )
    ...
    assert!(
        load(&db, &workspace, MemoryScope::Global, None)
            .candidates
            .is_empty()
    );
}
...
fn team_revocation_during_read_does_not_mix_roster_generations() {
    ...
    assert_eq!(ids(&corpus), [id.as_str()]);
    assert!(
        load(&db, &workspace, MemoryScope::Team, None)
            .candidates
            .is_empty()
    );
}
...
fn unavailable_scope_metadata_fails_closed_and_releases_the_snapshot() {
    ...
    db.execute_raw("ALTER TABLE memory_tags RENAME TO unavailable_tags")
        .unwrap();
    let error =
        load_scoped_ask_corpus(&db, &workspace, Utc::now(), MemoryScope::Global).unwrap_err();
...
fn global_tag_batches_do_not_truncate_the_candidate_corpus() {
    ...
    assert_eq!(
        corpus.candidates.last().unwrap().memory_id,
        format!("mem_{:026}", 260)
    );
}
...
fn team_scope_rejects_an_alternate_store_roster_without_falling_back() {
    ...
    member(
        &other,
        &workspace,
        "mbr_00000000000000000000000000000004",
        "Mallory",
        "active",
    );
    let error =
        load_scoped_ask_corpus(&other, &workspace, Utc::now(), MemoryScope::Team).unwrap_err();
    ...
    assert_eq!(
        load_current_ask_corpus(&other, &workspace, Utc::now())
            .unwrap()
            .candidates
            .len(),
        1
...
fn unavailable_team_roster_fails_closed_and_releases_the_snapshot() {
    ...
    db.execute_raw("ALTER TABLE team_members RENAME TO unavailable_roster")
        .unwrap();
