...
//!
//! * Lexicographic order of the string matches UUID byte order. For IDs
//!   generated by one process, `uuid::Uuid::now_v7` preserves
//!   oldest-first ordering across tight bursts.
...
//! markers). The UUID crate's monotonic tie-breaker is process-local:
//! cross-process or cross-host ordering must use explicit timestamp or
//! sequence columns, not ID sort order alone. No persistent generator
//! state is needed.
//!
...
impl<K: IdKind> Id<K> {
    ...
    /// process may be monotonic, but concurrent processes and hosts do
    /// not share a UUIDv7 counter or clock. Tests that require
...
mod tests {
    ...
    #[test]
    fn ids_from_now_v7_are_unique_and_monotonic_within_a_burst() {
        // The UUID crate's v7 generator is monotonic within one
        // process. That guarantee does not extend across ee processes
        // or hosts; persisted timestamps remain the cross-process
        // ordering source of truth.
        let mut ids = Vec::with_capacity(1_024);
        ...
        for pair in ids.windows(2) {
            assert!(
                pair[0] < pair[1],
                "UUIDv7 burst ordering regressed: {} !< {}",
                pair[0],
                pair[1]
            );
        }
