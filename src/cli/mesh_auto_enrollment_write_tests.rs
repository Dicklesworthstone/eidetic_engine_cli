...
use crate::db::{
    CreateWorkspaceInput, MeshLaneGrantMutationInput, MeshLaneGrantTargetAdapter,
...
fn candidate(key: char, stable: &str) -> AutoEnrollmentCandidate {
    AutoEnrollmentCandidate {
        ...
        stable_node_id: Some(stable.to_owned()),
        tailscale_ip: "100.64.0.2".to_owned(),
        magic_dns_name: None,
        hostname: format!("device-{key}"),
...
fn snapshot(connection: &DbConnection, old: &MeshForegroundSnapshot) -> MeshForegroundSnapshot {
    ...
    value.peers = connection
        .list_mesh_peers(&old.workspace_id)
        .unwrap()
        .iter()
        .map(MeshPeerRow::from)
        .collect();
    value
}
...
fn manual_migration_retains_the_refreshed_principal_and_revokes_only_unmatched_manual_peers() {
    ...
    assert_eq!(
        plan.peers_to_revoke.len(),
        2,
        "real planner includes the migrated old key"
    );
    materialize(
        &connection,
        &expected,
    ...
    let active = rows
        .iter()
        .find(|row| row.peer_id == retained.peer_id)
        .unwrap();
    assert!(
        active.enabled,
        "migration must not revoke the principal just refreshed"
    );
    assert_eq!(active.origin_node_id, retained.origin_node_id);
    let record: MeshPeerRecord =
        serde_json::from_str(active.policy_summary_json.as_deref().unwrap()).unwrap();
    assert_eq!(
        record.endpoint.tailscale_node_key,
        candidate('b', "stable-a").node_key
    );
    assert_eq!(record.trust_established_by, "tailscale_auto_enrollment");
    assert!(
        !rows
            .iter()
            .find(|row| row.peer_id == removed.peer_id)
            .unwrap()
            .enabled
    );
}
...
fn stale_discovery_cannot_resurrect_a_peer_revoked_by_another_connection() {
    ...
    let error = materialize(&connection, &expected, &discovery(), &plan, AFTER, || {
        invoked.set(true);
        Ok(())
    })
    .unwrap_err();
    ...
    assert!(
        !invoked.get(),
        "stale plans must not even persist configuration overrides"
    );
    assert_eq!(
        connection.list_mesh_peers(&empty.workspace_id).unwrap(),
        before
    );
}
...
fn competing_new_peer_or_policy_change_requires_replanning() {
    for add_peer in [false, true] {
        ...
        if add_peer {
            ...
            let mut policy: MeshPeerRecord =
                serde_json::from_str(first.policy_summary_json.as_deref().unwrap()).unwrap();
...
fn fresh_observation_timestamps_do_not_spuriously_invalidate_a_plan() {
    ...
    assert_eq!(
        connection
            .list_mesh_peers(&empty.workspace_id)
            .unwrap()
            .len(),
        1
    );
...
fn explicit_exclusion_of_the_old_key_cannot_be_evaded_by_stable_id_rotation() {
    ...
    plan.append_denylist_node_keys
        .push(candidate('a', "stable-a").node_key);
    let before = connection.list_mesh_peers(&empty.workspace_id).unwrap();
    let error = materialize(&connection, &expected, &discovery(), &plan, AFTER, || {
        panic!("exclusion must be decided before writing")
    })
    .unwrap_err();
...
fn stored_policy_cannot_override_an_authoritative_transport_device_binding() {
    ...
    connection
        .observe_mesh_peer_transport_identity(&ObserveMeshPeerTransportIdentityInput {
            workspace_id: empty.workspace_id.clone(),
            peer_id: first.peer_id.clone(),
            tailnet_id: TAILNET.to_owned(),
            stable_node_id: "authoritative-stable".to_owned(),
            current_node_pubkey: candidate('a', "").node_key,
...
fn failure_after_a_peer_write_rolls_back_peer_rows_and_consent_generation() {
    ...
    let plan = plan(
        &expected,
        vec![candidate('b', "stable-a"), candidate('c', "stable-c")],
        false,
    );
    let before = connection.list_mesh_peers(&empty.workspace_id).unwrap();
    let writes = Cell::new(0);
    let error = materialize_with_hooks(
        &connection,
        &expected,
        &discovery(),
        &plan,
    ...
    assert_eq!(
        connection
            .get_mesh_lane_grant_state(&empty.workspace_id, &first.peer_id)
            .unwrap(),
        Some(grant)
    );
...
fn failed_override_persistence_writes_no_peers_and_releases_the_transaction() {
    ...
    assert!(
        materialize(&connection, &empty, &discovery(), &plan, AFTER, || Err(
            denied("override failure")
        ))
        .is_err()
    );
    assert!(
...
fn audit_only_plans_never_open_a_write_transaction_or_call_hooks() {
    ...
    materialize(
        ...
        AFTER,
        || panic!("audit-only must not persist overrides"),
    )
    .unwrap();
}
...
fn matching_authoritative_device_can_rotate_without_reminting_the_peer() {
    ...
    let bound = connection
        .observe_mesh_peer_transport_identity(&ObserveMeshPeerTransportIdentityInput {
            ...
            peer_id: first.peer_id.clone(),
            tailnet_id: TAILNET.to_owned(),
            stable_node_id: "stable-a".to_owned(),
            current_node_pubkey: candidate('a', "stable-a").node_key,
            observed_at: Some(BEFORE.to_owned()),
        })
        .unwrap();
    let expected = snapshot(&connection, &empty);
    ...
    assert!(
        rows[0].transport_identity.is_none(),
        "changed enrollment still requires fresh LocalAPI authorization"
    );
}
