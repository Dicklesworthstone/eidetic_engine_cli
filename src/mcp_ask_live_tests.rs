...
fn denied(response: &Value) {
    ...
    assert!(
        !response.to_string().contains("private-canary"),
        "errors must not echo private constraint values"
    );
}
...
fn requested_authority_scope_is_not_dropped_when_paths_match() {
    ...
    for scope in ["verified", "self", "global", "team"] {
        let scoped = payload(&ask(
            root.path(),
            json!({"path":"src/new.rs", "memoryScope":scope}),
        ));
...
fn scope_revisions_and_retirement_are_visible_without_reindexing() {
    ...
    assert_ne!(
        ...
        initial["data"]["citations"][0]["entityRevision"]
    );
    db.execute_raw(&format!(
