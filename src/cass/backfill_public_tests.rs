    ...
    fn reimport_backfills_existing_transcripts_and_reconciles_snapshot_jobs() -> TestResult {
        for initially_include_spans in [false, true] {
            ...
            let client = CassClient::with_binary(binary.clone())
            ...
            ensure_equal(&first.spans_imported, &first_spans, "initial transcript count")?;
            ...
            ensure(new_job != old_job, "completed job cannot attest a newer transcript")?;
            ensure_equal(
            ...
            ensure_equal(&published.spans_imported, &0, "completed snapshot is idempotent")?;
            ...
            let metadata_only =
                import_cass_sessions(&client, &options).map_err(|e| e.to_string())?;
            ensure_equal(
