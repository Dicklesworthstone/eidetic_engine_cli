...
fn seeded_graph(node_count: usize) -> DiGraph {
    ...
    let density = 0.02_f64.max(12.0 / node_count.max(1) as f64).min(1.0);
    generator
        .fast_gnp_random_digraph(node_count, density, 59)
        .expect("hits benchmark graph should generate")
        .graph
}
...
fn run_once(node_count: usize) -> f64 {
    ...
    let scores = compute_hits(&graph).expect("hits benchmark succeeds");
    black_box(scores);
...
fn assert_budget(scale: usize, stats: QuickStats) {
    assert!(
        ...
        "graph_hits p50 exceeded for {scale} memories: {:.3}ms > {:.3}ms",
        stats.p50_ms,
        BUDGET_P50_MS
    );
    assert!(
        stats.p99_ms <= BUDGET_P99_MS,
        "graph_hits p99 exceeded for {scale} memories: {:.3}ms > {:.3}ms",
        stats.p99_ms,
        BUDGET_P99_MS
    );
}

fn bench_graph_hits(c: &mut Criterion) {
    ...
    for &scale in SCALES {
        group.bench_with_input(
            BenchmarkId::new("hits_centrality", format!("{scale}_memories")),
            &scale,
...
criterion_group!(benches, bench_graph_hits);
