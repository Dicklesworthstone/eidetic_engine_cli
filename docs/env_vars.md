...
| `EE_DAEMON_SEARCH_TIMEOUT_MS` | performance | integer milliseconds | `2000` | Override the bounded deadline `ee search --use-daemon` gives the daemon before falling back to in-process search. | Raise it on hosts where a cold or loaded daemon needs longer than the default to answer; the client still falls back rather than blocking forever (GH #37). |
