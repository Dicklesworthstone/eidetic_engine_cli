...
| `EE_DAEMON_SEARCH_TIMEOUT_MS` | performance | integer milliseconds | `5000` | Override the bounded deadline `ee search --use-daemon` gives the daemon before falling back to in-process search. | The default includes headroom above measured warm RPC latency. Missing sockets and warming daemons fall back immediately; stalled peers remain bounded (GH #37). |
| `EE_DAEMON_WARM` | performance | boolean flag | `on` | Set to `off` to stop a workspace-bound `ee daemon` from warming its search stack at startup. | Warm-up loads the tokenizer and embedding matrix once so `--use-daemon` amortises them instead of every CLI invocation rebuilding them (GH #37). |
