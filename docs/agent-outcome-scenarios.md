...
| `usr_privacy_export` | Privacy, redaction, export, and backup | `secret_redaction`, `privacy_export`, `dangerous_cleanup` | `ee remember --workspace <tmp> --level episodic --kind fact "<secret fixture>" --json`; `ee context "shareable support summary" --workspace <tmp> --json`; `ee backup create --workspace <tmp> --label fixture --json`; `ee backup verify <id> --json` | Redaction report, context golden with placeholders, backup record JSONL, backup manifest, secret absence assertions | `redaction_applied`, blocked secret storage, unsupported shareable export, backup verification failure | `eidetic_engine_cli-9sd5` / EE-USR-006, Gate 19 situation model where privacy affects routing, future backup/export beads | The agent can produce useful support or handoff artifacts without leaking secrets or private evidence. |
...
## Graph-Derived Agent Scenarios

These scenarios extend the core matrix with graph-accretion surfaces. They are
agent-facing because they answer "what should I inspect next?" before an agent
edits, curates, or trusts a memory cluster.

| ID | Journey | Command Sequence | Expected Signal | Degraded / Failure Branches | Owning Beads / Gates |
| --- | --- | --- | --- | --- | --- |
| `graph_insights_onboarding` | New agent maps workspace topology before choosing work | `ee insights --workspace <tmp> --json`; `ee insights --section bridges --workspace <tmp> --json`; `ee insights --section contradictionClusters --workspace <tmp> --json` | `ee.insights.v1` names available sections, bridge candidates, contradiction clusters, and next commands | workspace empty, stale graph snapshot, timed-out section, unknown section | `bd-t6wd`, `bd-bife.9`, F4 determinism harness |
| `graph_pack_dna_debug` | Agent explains why a context pack selected surprising memories | `ee context "prepare release" --workspace <tmp> --explain --json`; inspect `packDna`; follow with `ee why <id> --workspace <tmp> --json` | Pack DNA identifies dominator, community of mass, ego subgraph, and PPR neighbors without changing ordinary pack provenance | Pack DNA unavailable, graph budget exceeded, graph snapshot stale, no dominator | `bd-fdvt`, `bd-bife.9`, `ee.context.pack_dna.v1` |
| `graph_causal_failure_review` | Agent traces causal ancestry for a failure memory before proposing a fix | `ee why <failure-id> --workspace <tmp> --causal-explain --json`; `ee insights --section causalBottlenecks --workspace <tmp> --json` | Causal explanation returns paths, bottlenecks, min-cost or min-cut evidence, and degraded signals when causal evidence is missing | no causal evidence, causal cycle, stale causal snapshot, missing failure memory | `bd-qnfw`, `bd-bife.9`, `ee.why.causal.v1` |
| `graph_structural_curation` | Agent audits retention risk before curation disposition | `ee health --robot-insights --workspace <tmp> --json`; `ee curate disposition --workspace <tmp> --json`; `ee curate disposition --workspace <tmp> --no-structural-decay --json` | Structural health identifies k-truss support and contradiction clusters; curation reports structural adjustments for bridge and peripheral memories | structural health disabled, stale contradiction subgraph, single-shell graph baseline, no structural adjustments in opt-out mode | `bd-zx2v`, `bd-mvld`, `bd-bife.9`, ADR 0035 |
| `graph_proximity_review` | Agent decides whether two memories should be reviewed or packed together | `ee proximity <id-a> <id-b> --workspace <tmp> --json`; `ee insights --section proximityHotspots --workspace <tmp> --json`; optional `ee context "<task>" --workspace <tmp> --ppr-weight 0.5 --explain --json` | Proximity reports min-cut, interpretation, tree path, and hotspot ranking for tightly coupled memories | endpoints missing, disconnected components, stale Gomory-Hu cache, sampled large graph | `bd-5vqr`, `bd-ov09`, `bd-bife.9`, `ee.proximity.v1` |
