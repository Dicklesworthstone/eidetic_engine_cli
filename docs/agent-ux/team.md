...
(`ee.team.budgets.v1`).
`ee team doctor` `broker_port` compares the folded team hello port
(`ee team port show`) to `EE_MESH_HELLO_PORT` (default 41888).
`ee team port migrate --to <port> --confirm` appends `teamPortMigrated`
and rewrites enrolled peer locators; genesis hash, pair keys, and
grants stay untouched. `whois` documents the accept
