...
| `mcp`                | active     | `[]`                                     | Gates the optional stdio/server adapter module at `src/lib.rs:24`. The CLI discovery surface `ee mcp manifest` remains available in default builds and reports `mcp_feature_disabled` as a capability gap when the adapter is not compiled. Opt-in via `cargo install eidetic-engine --features mcp` per README §Agent Harness Integration. | `bd-17c65` (MCP adapter) | 0.1.0       |
