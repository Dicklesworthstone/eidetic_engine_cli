...
| `mcp`                | active     | `[]`                                     | Gates the `pub mod mcp;` declaration at `src/lib.rs:23` — the entire MCP adapter (2.6k+ LOC). Opt-in via `cargo install eidetic-engine --features mcp` per README §Agent Harness Integration. | `bd-17c65` (MCP adapter) | 0.1.0       |
