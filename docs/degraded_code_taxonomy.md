...
| `rerank_model_unavailable` | search rerank posture | Permanent capability posture when no usable local reranker is registered and this build has no bundled/network fetch. Search and doctor provide the exact verified offline-import command with a caller-supplied artifact path. One-shot search carries one structured advisory, while a long-lived daemon session emits it once and then reports cumulative suppression counts under `rerank.advisorySummary`. Registered-artifact registry/load failures remain transient query degradations on every affected response. |
...
| `search_index_large_gap` | medium | bd-index-auto-freshness-m5kwf (INDEX) |
