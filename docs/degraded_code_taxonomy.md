...
| `rerank_model_unavailable` | search rerank posture | Permanent capability posture when no usable local reranker is registered and this build has no bundled/network fetch; registered-artifact registry/load failures remain transient query degradations. Permanent absence is structured under `rerank.advisory` and has a companion doctor capability gap without emitting this degraded code. |
