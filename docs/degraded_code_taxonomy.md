...
| `rerank_model_unavailable` | search rerank posture | Permanent capability posture when no usable local reranker is registered and this build has no bundled/network fetch. Search and doctor state that automatic repair is unavailable; they do not mislabel a caller-supplied path template as executable recovery. One-shot search carries one structured advisory, while long-lived daemon, `ee serve`, and MCP stdio process sessions emit it once and then report cumulative suppression counts under `rerank.advisorySummary`. Registered-artifact registry/load failures remain transient query degradations on every affected response. |
...
| `global_index_unavailable` | medium | bd-reality-core-convergence-1azkt.4 |
...
| `memory_drift_report_unavailable` | warning | bd-1xpq9 (DRIFT) |
...
#### Mesh authenticated transport and responder broker (5)
| Code | Severity | Bead |
|------|----------|------|
| `mesh_bootstrap_identity_unverified` | high | bd-tc-epic-qzk7o.3.3 (TC) |
| `mesh_body_cache_lifecycle_failed` | high | bd-tc-epic-qzk7o T5.9 (TC) |
