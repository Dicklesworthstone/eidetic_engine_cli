...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `asupersync` | `ee-runtime` | registry `=0.5.0`, `default-features = false`, `tracing-integration` | `test-internals` is dev-only; `deterministic-mode` is test-only; `sqlite` is blocked because it can expose `rusqlite` | `runtime_status_reports_asupersync_engine` | `runtime_unavailable` | `runtime.engine`, `runtime.profile`, `runtime.async_boundary` | `ee status --json` |
| `frankensqlite` | `ee-db` | registry `0.4.0`; `fsqlite` has `default-features = false` with `json` enabled | `fts5` is owned by the Frankensearch lexical profile; `rtree`, `session`, `icu`, and `misc` are not default database release gates | `default_feature_tree_excludes_forbidden_crates` and migration tests | `storage_unavailable` | `capabilities.storage`, `degraded[].code` | `ee doctor --json` |
| `sqlmodel_rust` | `ee-db` | registry `sqlmodel-core` and `sqlmodel-frankensqlite` `0.5.0` | `c-sqlite-tests` is blocked because it is parity-only and can expose `rusqlite` | `migration_sequence_is_contiguous` and repository tests | `storage_unavailable` | `capabilities.storage`, `database.schema_version` | `ee status --json` |
| `frankensearch` | `ee-search` | registry `0.6.0`, `default-features = false`, `hash`, `storage`, `model2vec`, `download`, `lexical-tantivy`, `fts5`, `rerank`, and `native` through `ee` features | `fastembed` and high-quality embed profiles are blocked until forbidden network/runtime crates are absent; the default download path is allowed because it uses asupersync HTTP, not `reqwest`/`hyper`/Tokio | search/index smoke tests with hash, bundled Model2Vec, rerank posture, and deterministic hash fallback | `search_unavailable`, `embed_model_unavailable` | `capabilities.search`, `index.generation`, `data.embedding`, `degraded[].code` | `ee index status --json` |
| `franken_networkx` | `ee-graph` | registry `0.3.0`; the default-on `graph` profile enables `fnx-runtime/asupersync-integration`, `fnx-classes`, and `fnx-algorithms` | `differential-networkx` is test-only; `ftui-integration` is not part of `ee` | graph projection and centrality tests | `graph_unavailable` | `capabilities.graph`, `graph.snapshot_generation` | `ee diag graph --json` |
...
| `toon_rust` | `ee-output` | registry package `tru` `=0.2.4`, `default-features = false` | TOON is a renderer only; JSON remains the canonical schema surface | TOON renderer round-trip/golden parity tests | `toon_unavailable` | `capabilities.output.toon` | `ee status --json` |
...
- Release and source builds resolve registry versions and checksums from
  `Cargo.lock`. No sibling paths or Cargo patches are required.
  `franken-stack.lock` retains the upstream source revisions qualified for
  publication. Checkout helpers skip sibling setup for registry-only manifests.
