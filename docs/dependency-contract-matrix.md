...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `asupersync` | `ee-runtime` | registry `=0.4.10`, `default-features = false`, `tracing-integration` | `test-internals` is dev-only; `deterministic-mode` is test-only; `sqlite` is blocked because it can expose `rusqlite` | `runtime_status_reports_asupersync_engine` | `runtime_unavailable` | `runtime.engine`, `runtime.profile`, `runtime.async_boundary` | `ee status --json` |
