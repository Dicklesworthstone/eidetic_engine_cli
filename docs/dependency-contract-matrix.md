...
| `frankensqlite` | `ee-db` | local `0.3.11` `/data/projects/frankensqlite` patches; `fsqlite` has `default-features = false` with `json` enabled | `fts5` is owned by the Frankensearch lexical profile; `rtree`, `session`, `icu`, and `misc` are not default database release gates | `default_feature_tree_excludes_forbidden_crates` and migration tests | `storage_unavailable` | `capabilities.storage`, `degraded[].code` | `ee doctor --json` |
| `sqlmodel_rust` | `ee-db` | `sqlmodel-core` and `sqlmodel-frankensqlite` `0.4.1` via local paths | `c-sqlite-tests` is blocked because it is parity-only and can expose `rusqlite` | `migration_sequence_is_contiguous` and repository tests | `storage_unavailable` | `capabilities.storage`, `database.schema_version` | `ee status --json` |
| `frankensearch` | `ee-search` | local `0.4.0` `/data/projects/frankensearch`, `default-features = false`, `hash`, `storage`, `model2vec`, `download`, `lexical-tantivy`, `fts5`, and `rerank` through `ee` features | `fastembed` and high-quality embed profiles are blocked until forbidden network/runtime crates are absent; the default download path is allowed because it uses asupersync HTTP, not `reqwest`/`hyper`/Tokio | search/index smoke tests with hash, bundled Model2Vec, rerank posture, and deterministic hash fallback | `search_unavailable`, `embed_model_unavailable` | `capabilities.search`, `index.generation`, `data.embedding`, `degraded[].code` | `ee index status --json` |
