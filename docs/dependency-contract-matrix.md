...
| `frankensearch` | `ee-search` | local `/data/projects/frankensearch`, `default-features = false`, `hash`, `storage`, `model2vec`, `download`, `lexical`, `fts5`, and `rerank` through `ee` features | `fastembed`, `api`, and high-quality embed profiles are blocked until forbidden network/runtime crates are absent; the default download path is allowed because it uses asupersync HTTP, not `reqwest`/`hyper`/Tokio | search/index smoke tests with hash and bundled Model2Vec posture | `search_unavailable`, `embed_model_unavailable` | `capabilities.search`, `index.generation`, `data.embedding`, `degraded[].code` | `ee index status --json` |
