...
186 stable command paths returned by the command-path extractor.

E2E audit record for this inventory:

- Command source: `src/cli/mod.rs`
- Command path count: 186
...
| `eval` | mechanical CLI | `eidetic_engine_cli-uiy3`, ADR 0011 | evaluation | deterministic fixture registry, retrieval metrics, and pack-quality reports | none | storage/search/config errors only; pack-quality regressions use exit code 9 with a parseable response | class=read_only; run/list read fixture files and build temporary derived indexes only; no durable mutation | runtime=bounded_read; budget=fixture_runner; cancel=checkpoint; partial=temp_index_discard; outcome=success_or_eval_failure_or_degraded | eval fixtures and goldens | `ee.response.v1` with `ee.eval.report.v1` or `ee.eval.pack_quality_report.v1` | fixture/e2e |
...
| `playbook` | mechanical CLI | `eidetic_engine_cli-ivkj`, ADR 0011 | curation/rules | semantic memories, curation candidates, procedural-rule apply path, and audit log | none | `curation_store_unavailable` | class=audited_mutation; extract writes rule candidates and audit rows in one DB transaction per candidate; idempotency=retry skips duplicate proposed rule content; dry-run writes nothing | runtime=bounded_write; budget=db_scan_and_candidate_txn; cancel=pre_commit_checkpoint; partial=rollback_with_audit; outcome=success_or_storage_error | playbook extraction unit flow | `ee.response.v1` | unit plus command-matrix contract |
...
| `swarm brief` | mechanical CLI | `eidetic_engine_cli-v731`, ADR 0011 | agent coordination preflight | read-only git, Beads, BV, Agent Mail snapshot, RCH, host profile, and agent inventory probes | none | source-specific degraded records such as `git_unavailable`, `beads_unavailable`, `bv_unavailable`, `agent_mail_unavailable`, `rch_unavailable`, and `agent_status_unavailable` | class=read_only; idempotency=fully_idempotent; mutation=none; selected sources may be skipped or degraded but never claimed, reserved, closed, built, or committed | runtime=bounded_read; budget=source_probe; cancel=between_bounded_probes; partial=none; outcome=success_or_degraded_or_unsatisfied_required_sources | `eidetic_engine_cli-v731` CLI parser/output tests and `eidetic_engine_cli-pdav` schema/goldens | `ee.response.v1` payload `ee.swarm.brief.v1` | unit plus command-matrix and follow-on contract/golden |
...
| curation-rules | Curation and rules | List, validate, apply, accept, reject, snooze, merge, extract playbook rule candidates, and protect procedural candidates through audited mutations. | `curate candidates`, `curate validate`, `curate apply`, `curate accept`, `curate reject`, `curate snooze`, `curate merge`, `curate disposition`, `playbook extract`, `rule add`, `rule list`, `rule show`, `rule protect` | curation skill governed by `skills/ee-skill-standards/SKILL.md` only for judgment-heavy summaries before candidate mutation. | `curation_store_unavailable`, `rule_store_unavailable`, or `outcome_store_unavailable`; no silent promotion. | `ee curate candidates --workspace . --json`, `ee doctor --json`. | `eidetic_engine_cli-ynzg`, `eidetic_engine_cli-s43e`, `eidetic_engine_cli-g9dq` | curation mutation contracts plus redacted handoff fixtures | covered; no feature was dropped because candidate lifecycle writes stay mechanical and judgment is explicitly skill-scoped. |
...
| privacy-trust | Privacy and Trust | Redact secrets, assign trust classes, quarantine prompt-injection candidates, and expose provenance/audit trails. | `remember`, `outcome`, `curate candidates`, `rule protect`, `handoff create`, `handoff preview`, `why` | handoff review and curation skills governed by `skills/ee-skill-standards/SKILL.md` operate only on redacted evidence bundles. | `handoff_unavailable`, `curation_store_unavailable`, `outcome_store_unavailable`, or policy-denied mutation; suspicious memories quarantine instead of promoting. | `ee handoff preview --json`, `ee curate candidates --json`, `ee why <memory-id> --json`. | `eidetic_engine_cli-g9dq`, `eidetic_engine_cli-ynzg`, `eidetic_engine_cli-s43e`, `eidetic_engine_cli-7pq.1` | redaction, quarantine, trust, and outcome contract fixtures | covered; no feature was dropped because privacy/trust behavior is preserved as mechanical policy plus audited skill input. |
...
| deterministic evaluation entrypoint | `eval run`, `eval list` | deterministic fixture registry, retrieval metrics, and pack-quality comparison reports | class=read_only; run/list read fixture files and build temporary indexes only; no durable mutation | runtime=bounded_read; budget=fixture_runner; cancel=checkpoint; partial=temp_index_discard; outcome=success_or_eval_failure_or_degraded | fixture config/search errors and pack-quality eval-failure exit code; no unavailable sentinel | eval run/list degraded-honesty contracts plus pack-quality fixture e2e |
...
| `eval run`, `eval list` | `src/cli/mod.rs`, `src/eval/runner.rs` | Fixture-backed retrieval and pack-quality reports are read-only; stale output renderers remain legacy helpers when called without a runner report. | keep mechanical; prefer CLI fixture runner paths |
...
| Eval output renderers | `src/output/mod.rs:4092`, `src/output/mod.rs:4103`, `src/output/mod.rs:4290` | Legacy renderer fallback can still create an empty report if called without a runner report. | Keep public `eval run/list` on the fixture-backed CLI path; remove or constrain legacy fallback before exposing new renderer entrypoints. |
