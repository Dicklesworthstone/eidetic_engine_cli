...
165 stable command paths returned by the command-path extractor.

E2E audit record for this inventory:

- Command source: `src/cli/mod.rs`
- Command path count: 165
...
| `causal` | implements-surface | `eidetic_engine_cli-hnrm`, ADR 0022 | causal evidence review | `causal_evidence` ledger rows linked to memory evidence | causal credit review skill | `causal_evidence_unavailable` for empty ledgers | class=mixed; trace/estimate/compare are read-only evidence projections; promote-plan writes an audited curation candidate with chain provenance | runtime=bounded_query; budget=evidence_query; cancel=checkpoint; partial=no mutation before promote-plan candidate write | causal ledger contract tests | `ee.causal.trace.v1`, `ee.causal.estimate.v1`, `ee.causal.compare.v1`, `ee.causal.promote_plan.v1` | core tests plus CLI contract and no-fake-reasoning |
...
| `recorder tail`, `recorder follow` | `src/core/recorder.rs` + V027 `recorder_events` | Read-only streaming over persisted recorder events. Tail returns deterministic windows; follow emits one JSON event per line. | implemented by `eidetic_engine_cli-qow7` |
