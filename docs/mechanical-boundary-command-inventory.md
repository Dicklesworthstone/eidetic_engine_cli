...
| manual memory write/read | `remember`, `memory list`, `memory show`, `memory history` | memory DB rows, provenance, revision history | class=audited_mutation for `remember`; read-only for list/show/history; rollback with audit | runtime=bounded_write; budget=db_transaction; cancel=pre_commit_checkpoint; partial=rollback_with_audit; outcome=success_or_storage_error | `storage`; repair via `ee status --json` and DB diagnostics | walking-skeleton e2e plus JSON/golden response coverage |
