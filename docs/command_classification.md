...
| `journal *` | `core::journal` | Mechanical | DB | Append-only observation journal (append/list/show) plus deterministic extractive distillation into curation candidates (distill; dry-run default, --apply writes) |
...
| `situation show/explain` | `src/core/situation.rs` `situation_records` readers | Missing persisted situation storage could be misreported as a generated situation or the retired unavailable sentinel. | Reads stored `situation_records`; unknown IDs return `not_found`, missing/unreadable databases return storage errors, and `situation_decisioning_unavailable` remains only as a retired fixture tombstone. |
...
| `eval run/list` | `src/cli/mod.rs` fixture runner | No-scenario stub success. | Reads deterministic fixture metadata and emits `ee.eval.report.v2` metrics or fixture lists; DH-17 guards against the removed `eval_fixtures_unavailable` sentinel. |
