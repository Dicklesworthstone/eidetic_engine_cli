...
| T6.1 steward | **Proven** | Isolated 2026-08-14: membership, projects, orphaned Next pair, inbound memory rematerialize, SingleDocument index enqueue, inbound body placeholders, join peer enroll, and grant-gated fetch retry. `enroll_team_pair_peer_uses_the_pair_key_handle` 13m 58s + 53.11s exit 0. `retry_pending_team_body_fetches_requires_a_body_lane_grant` 18m 44s + 57.20s exit 0. Join enrolls the inviter under the pair-key handle so EventFetch can name a live TCP endpoint. Retry calls fetch only when the durable body lane is Allow, then applies nonce-checked bytes. | Authenticated BodyFetch still needs a distinct responder workspace plus pair-key session. Unsigned EventFetch can contact the enrolled invite endpoint. |
...
18m 44s + 57.20s, both exit 0. Windows inbound listen and a
