...
Campaign: `bd-tc-epic-qzk7o`. Proof host: isolated worker-c
...
host `ubuntu@198.51.100.12` is pubkey-denied as of 2026-08-17.
...
| --- | --- | --- |
| Two-human Tailscale US-4 soak | `bd-tc-epic-qzk7o.3.8` (T2.6) **closed 2026-08-17** | Live two-host tailnet soak: worker-c origin `100.64.0.10:41888` + worker-f joiner `100.64.0.20`. Invite/join over Tailscale IPs; authenticated EventFetch after pair-key+LocalAPI (`8055b5b1`); origin body-lane grant gen 1; joiner `mesh sync --once` `contactedPeers=true` / `health=synced` with no `mesh_sync_once_network_deferred`. `ee search`/`ee pack --memory-scope team` returned `US4-SOAK-MARKER Acme Corp Q3 analysis ...`. **Amendment 2026-09-17 (Branch B, approved by session operator via WildBluff; bead `bd-reality-core-convergence-1azkt.8`):** the v1 acceptance boundary is two independent tailnet hosts, not two distinct human operators; this artifact satisfies it as-is. Residual risk: distinct-operator ceremony/usability is unproven; the two-human soak is a post-v1 follow-up requiring an owner before any roadmap promise. |
| T5.7 publication fence | `bd-d67os.28` **closed 2026-08-17**; `.6.7` **closed 2026-08-17** | NavyLotus protocol (no fence rewrite). Isolated worker-c pinned tree `24193e89` 2026-08-17: `single_processor_never_stamps_post_snapshot_commit_current` 111.39s EXIT 0; `limited_coalesced_processor_fences_unclaimed_and_post_snapshot_jobs` 52.49s EXIT 0. Prior RCH lib tests 2/2 in 42.56s on worker-01 at the same commit. Multiprocess E2E `multiprocess_write_after_source_snapshot_is_present_or_explicitly_stale` 334.26s EXIT 0 on worker-c: 256-doc snapshot, `index status` health=stale with rebuild repair, second writer absent from search, coalesce repair to ready, unique phrase then present. Search may disclose `search_index_degraded` when its freshness probe fails under hash_fallback; that path still refuses to claim Current. Team-scale coalesce remains `inbound_index_jobs_coalesce_under_amplification_cap` (500-row → 1 Incremental job). RCH E2E on worker-04 compiled 16m07s then failed as environment (`ee.write.lock` 38s no-progress / worker disk). |
| Windows-host DACL / inbound crash / key-path soak | `bd-tc-epic-qzk7o.12` (`.2.4` **closed 2026-08-17**; `.12` **closed 2026-08-17**) | `.2.4` live: windows-host-1 `ee 0.13.1` `14:53` `target\release\ee.exe` `team create` on `C:\Users\dev\ee-tc-win-soak5`. `.ee\keys\mesh` DACL `D:P` TokenUser (`WINDOWS-HOST-1\dev`) + SYSTEM, no inherited ACEs; signing record same. `.ee` ancestors stay inherited so `ee.db` remains writable (`19dcfd8c`). Reparse junction on soak2-reparse refused (`path is (or traverses) a symbolic link`). `.12` live 2026-08-17: soak6 `C:\Users\dev\ee-tc-win-soak6` store row `wsp_700J9NPX8BKY4TQF8XJDA2RHSS` (`\\?\C:\...`); invite `66804dc593b6a64ac56eb04549d75595` redeemed by worker-c `/home/ubuntu/ee-tc-win-soak6b-join` over Tailscale `100.64.0.10` → `100.64.0.42:41888`. Enroll writes `mesh_peers` on the store row (`86541697`). `hello-responder run` `05:56` `ee.exe` binds `100.64.0.42:41888` (`c7cd4702` routed listen + `d4a159d2` TeamJoin WhoIs). worker-c `ee mesh sync --once` `contactedPeers=true` / `health=synced` / `importedEventCount=1`. Kill PID 23208 only, restart PID 19864; inbound hello+sync still returns genesis `blake3:f7a3a19d83518780777a8ff075e9f3b30088e7cb885f922759a5c28129b35658`. Post-restart unsigned hello took 4–8s on Tailscale, so default foreground TCP sync timeout is 15s and `--time-budget-ms` defaults to 20000. |
| Production IdP vendor soak | `bd-tc-epic-qzk7o.8.8` **closed 2026-08-17**; `bd-tc-followup-oo7d2.8` **not built 2026-08-18** | Fake-IdP RS256 + live `identity_attest` is the identity product (TC-D19). No Entra/Okta/Google secretless public client exists here. A vendor tenant is a new epic when one exists. |
| Manifest roles / quorum | `bd-tc-followup-oo7d2.2` **not built 2026-08-18** | Equal-member remove-wins + doctor delegated-member review. Quorum would stall small trusted teams. New epic only for IT-owned multi-admin teams (TC-D19). |
| Selective-sync subscriptions | `bd-tc-followup-oo7d2.4` **not built 2026-08-18** | Membership is the sync scope. Origin publication + grants already filter. Contractors are not invited (TC-D19). |
| Program closeout | `bd-tc-epic-qzk7o.7.7` (T6.7) **closed 2026-08-17**; `bd-tc-followup-oo7d2` **closed 2026-08-18** | Every tc-* implement child is closed. Post-v1: .1 .3 .7 .9 shipped; .2 .4 .5 .6 .8 decided not-to-build (TC-D15/D16/D19). Do not start `bd-1nl13`. |
| Windows local control transport | `bd-tc-followup-oo7d2.9` **closed 2026-08-18** | Loopback TCP + owner-only `%LOCALAPPDATA%\eidetic-engine\mesh-responder.control` endpoint file (`ee.mesh.responder.control.endpoint.v1`). Unix UDS unchanged. Isolated worker-c Unix `cargo test --lib responder_broker` 13 passed. windows-host-1 native 14 passed including `windows_control_endpoint_publishes_and_status_submits` and verbatim `\\?\` owner-safe path. Live soak `C:\Users\dev\ee-tc-win-control-soak` `WinControlSoak`: `hello-responder run --port 41889` bound `100.64.0.42:41889`; `status --json` `running=true` `listenAddress=100.64.0.42:41889` `degraded=[]`; `register --peer peer_8172d940cc571dda65183dd6325abbf2 --port 41889` `ok=true` `registeredRoutes=1`. CLI register was Unix-only (`PlatformUnsupported`) until the Windows handler compiled; owner-safe walk now strips `\\?\` so canonicalize()d paths pass. Named-pipe listen is rejected until a safe-Rust adapter exists (TC-D17); loopback TCP is the Windows control transport. |
| Encrypted credential backup | `bd-tc-followup-oo7d2.7` **closed 2026-08-18** | First-class `ee team credentials backup\|restore --passphrase-stdin`. Envelope schema `ee.mesh.credentials.backup.v1`; payload sealed with blake3-derive-key + ring CHACHA20-POLY1305. Written through `SecureLocalDir` under `.ee/keys/mesh-credential-backup/` (Unix 0600 / Windows TokenUser+SYSTEM DACL). Isolated worker-c 2026-08-18 `/home/ubuntu/ee-mesh-campaign-verify-pinned`: `cargo test --lib credential_backup` 6 passed in 1.32s. Live CLI soak `/home/ubuntu/ee-cred-backup-soak-20260818`: `team create CredBackupSoak` then backup `storePresent=true pairCount=0 signingCount=1`, restore wrote `signing.node_987f14217476edd62346865fa87baacf.current.json` mode 600, envelope 600 / dir 700, wrong passphrase fail-closed `credential backup could not be decrypted`. Ordinary `ee backup` still redacts `peerCredentials`. |
| Multi-team in one store | `bd-tc-followup-oo7d2.5` **rejected 2026-08-18** | Two teams means two workspaces (TC-D15). The broker already multiplexes workspace routes on one host-wide port. A second `teamCreated` in one store is not a leftover hole. |
| Native rustls mesh backend | `bd-tc-followup-oo7d2.6` **rejected 2026-08-18** | Mesh security is Tailscale + Frame v2 pair-key MAC + Ed25519 origin (TC-D16). rustls is transitive via asupersync only. IdP HTTPS stays constrained curl. `rg rustls src/mesh` is empty on purpose. |
| Versioned team-port migration | `bd-tc-followup-oo7d2.1` **shipped 2026-08-18** | First-class `ee team port show` / `ee team port migrate --to N --confirm`. Appends signed `teamPortMigrated` (does not rewrite genesis). Isolated worker-c `/home/ubuntu/ee-mesh-campaign-verify-pinned/eidetic_engine_cli-port` at `10a00f7b` plus overlay: `cargo test --lib migrate_team_port` 2 passed in 43.52s (`folds_without_rewriting_genesis`, `rewrites_enrolled_peer_locator_and_leaves_pair_key`). Live CLI soak `/home/ubuntu/ee-port-migrate-soak-20260818` `PortMigrateSoak` `team_28a364dd67358ca114001f02b4` genesis `blake3:206247e353e9e586d380009b53fd34a9425cb9c5ba81f8b7d18f482082118223`: show gen 1 port 41888; migrate without `--confirm` usage exit 1; `--to 80` nonprivileged refuse; `--to 41888` same-port refuse; `--to 41999 --confirm` `migrated=true` `currentHelloPort=41999` `previousHelloPort=41888` `portGeneration=2` same genesis hash `pairKeysUnchanged=true` `grantsUnchanged=true`. Status `helloPort=41999`. Invite `helloPort=41999`. Doctor `broker_port` warning repairs with `ee team port migrate --to 41888 --confirm` or `EE_MESH_HELLO_PORT=41999`. `ee mesh hello-responder run` / daemon owner / register bind `local_hello_bind_port`: `--port` then `EE_MESH_HELLO_PORT` then the folded team port, so a migrate to 41999 is listened on without rewriting genesis. Joiner `ee mesh sync --once` and join first-sync apply imported `teamPortMigrated` onto enrolled locators without requiring `ee team members reconcile`; pair keys stay closed. |
...
| T5.9 Windows SID/DACL/reparse | **Live key-path + inbound crash/restart proven on windows-host-1** | Isolated `x86_64-pc-windows-gnu --lib` still compiles. Live 2026-08-17: `56f2cea8` in-place harden, `909a1e7a` TokenUser+writable dir handle, `19dcfd8c` leaf-only DACL. Soak5 `team create` `WinSoak5` `team_540f2c013aee8acd0022ba40eb`; mesh DACL `D:P(A;;FA;;;S-1-5-21-...-1001)(A;;FA;;;SY)`; reparse junction denied. Soak6 enroll + hello-responder crash/restart: worker-c joiner `ee mesh sync --once` then kill/restart Windows PID; inbound hello+sync still serves genesis. | Tailscale WhoIs remains Unix; TeamJoin inbound is the Windows path |
...
| T6.4 doctor | **Proven** | Isolated 2026-08-14: `inspect_team_health_reports_no_team_then_ok_then_paused` 65.39s exit 0. `broker_port` compares the folded team hello port to `EE_MESH_HELLO_PORT`; mismatch is a warning with `ee team port migrate --to <configured> --confirm`. `whois` no longer claims a live Tailscale probe. | Tailscale LocalAPI WhoIs remains Unix; TeamJoin inbound is cross-platform |
