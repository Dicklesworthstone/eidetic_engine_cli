...
| T5.10 US-6 E2E | **Proven (Unix product harness)** | Token bind/drift/expiry/wrong-store, unlinkable previews, crash lifecycle, `already_redacted` publish, redact-over-exact refuse, `ee team share bodies --representation`, BodyFetch requires durable Body-lane Allow, grant then revoke flips the fetch gate, substituted cache bytes stay metadata-only | No Windows-host crash killer |
...
| T6.4 doctor | **Landed** | genesis/posture/members/steward/admission/key_store/body_cache/store_auth/idp/timer/daemon_service/broker_port/client_only/whois/body_cache_lifecycle/index_rematerialization/origin_outbox | — |
...
`origin_outbox` + rematerialization doctor rows, and a reviewed
`HardenedWindows` secure-file adapter in tree. Windows inbound
