...
| T5.1–T5.8 pack scope, activity, attribution | **Landed; inbound body hydrates and indexes for team search** | Authorized BodyFetch replaces the `[ee.team.history]` stub with UTF-8 teammate text. Apply and `ee team steward once` drain the SingleDocument job so `ee search --memory-scope team` can recall it even when the joiner has no prior local index. Isolated 2026-08-15: `hydrate_inbound_team_memory_body_replaces_history_stub` 32m 07s + 63.77s EXIT 0. Isolated 2026-08-15: `hydrated_team_memory_is_searchable_under_team_scope` 10m 53s compile + 194.00s test EXIT 0 — `ee search --memory-scope team` returns the hydrated teammate memory after apply/steward drain the inbound index job. | Two-human Tailscale search/pack e2e remains an environment remainder |
...
Tailscale LocalAPI WhoIs stays Unix. Do not self-close beads from this ledger.
