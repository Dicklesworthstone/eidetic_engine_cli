...
| T5.1–T5.8 pack scope, activity, attribution | **Landed; search/pack JSON emit teamProvenance** | Authorized BodyFetch hydrates and indexes teammate text. Search hits, pack markdown, `ee pack --json` items, and pack JSONL carry `teamProvenance`. Isolated 2026-08-15: hydrate + searchable + provenance unit tests EXIT 0. | Two-human Tailscale search/pack e2e remains an environment remainder |
