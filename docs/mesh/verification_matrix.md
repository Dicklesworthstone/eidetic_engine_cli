...
| T5.1–T5.8 pack scope, activity, attribution | **Landed; search/pack/activity recall teammate text** | Authorized BodyFetch hydrates and indexes teammate text. Search hits, pack markdown, `ee pack --json` items, and pack JSONL carry `teamProvenance`. Apply/steward hydrate leftover stubs from already-available cache. `ee team activity` attributes inbound projections with member name, origin `producedAt`, and `bodyAvailable` after hydrate. Isolated 2026-08-15: hydrate + searchable + provenance unit tests EXIT 0. Isolated 2026-08-15 pack-recall / retry-hydrate / activity attribution proofs pending this run. | Two-human Tailscale search/pack e2e remains an environment remainder |
