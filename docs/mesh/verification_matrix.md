...
| Windows local control transport | `bd-tc-followup-oo7d2.9` **in progress** | Loopback TCP + owner-only `%LOCALAPPDATA%\eidetic-engine\mesh-responder.control` endpoint file. `listen_control` / `submit` / `hello-responder register|status` now compile for Windows. Isolated CSS Unix `cargo test --lib responder_broker` 13 passed. Windows-gnu cross on CSS lacks `x86_64-w64-mingw32-gcc`. Live SurfaceBookJE soak still pending. |
