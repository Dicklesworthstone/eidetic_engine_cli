...
`ubuntu@38.242.134.66` (trees `/tmp/ee-mesh-verify` then
`/home/ubuntu/ee-mesh-campaign-verify`; no Mac local Cargo).
This is a live Unix EE-to-EE ledger, not a bead-closing ceremony.

Unix product is shipped. The tracker stay-open set is the real remainder,
not unbuilt transport:
...
| Windows-host DACL / inbound crash / key-path soak | `bd-tc-epic-qzk7o.12` (`.2.4` **closed 2026-08-17**) | `.2.4` live: SurfaceBookJE `ee 0.13.1` `14:53` `target\release\ee.exe` `team create` on `C:\Users\jeffr\ee-tc-win-soak5`. `.ee\keys\mesh` DACL `D:P` TokenUser (`SURFACEBOOKJE\jeffr`) + SYSTEM, no inherited ACEs; signing record same. `.ee` ancestors stay inherited so `ee.db` remains writable (`19dcfd8c`). Reparse junction on soak2-reparse refused (`path is (or traverses) a symbolic link`). `.12` remainder: css joined over Tailscale `100.120.65.94:41888` ← `100.90.148.85` (`invite wait` granted, member `WinJoinCss` active). `hello-responder run` then misses `mesh_peers` because `\\?\C:\...` vs `C:\...` hash to different `wsp_*` ids. |
...
| Program closeout | `bd-tc-epic-qzk7o.7.7` (T6.7) | Closes only after the remainders above; do not invent a ceremony |

Do not start `bd-1nl13`. Do not close the epic until the remainder children close.
...
| T5.9 Windows SID/DACL/reparse | **Live key-path soak proven on SurfaceBookJE** | Isolated `x86_64-pc-windows-gnu --lib` still compiles. Live 2026-08-17: `56f2cea8` in-place harden, `909a1e7a` TokenUser+writable dir handle, `19dcfd8c` leaf-only DACL. Soak5 `team create` `WinSoak5` `team_540f2c013aee8acd0022ba40eb`; mesh DACL `D:P(A;;FA;;;S-1-5-21-...-1001)(A;;FA;;;SY)`; reparse junction denied. css redeemed invite over Tailscale TCP 41888. | `.12` hello-responder crash/restart still open: `\\?\` vs `C:\` workspace-id drift hides `mesh_peers`. Tailscale WhoIs remains Unix; TeamJoin inbound is the Windows path |
