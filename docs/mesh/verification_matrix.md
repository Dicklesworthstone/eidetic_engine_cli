...
| T5.1–T5.8 pack scope, activity, attribution | **Landed; search/pack now emit teamProvenance** | Authorized BodyFetch hydrates and indexes teammate text. Team-scoped search hits carry `teamProvenance` (`memberDisplayName`, member-attested `producedAt`); pack markdown prints `· from <member> · <producedAt>`. Isolated 2026-08-15: hydrate + searchable tests EXIT 0. Isolated 2026-08-15: `team_provenance_from_peer_human_attested_memory` 20m 13s + 0.02s EXIT 0; `team_pack_attribution_suffix_renders_member_and_origin_time` 10.12s + 0.00s EXIT 0; `project_inbound_team_memory_writes_a_metadata_stub` 58.97s EXIT 0. | Two-human Tailscale search/pack e2e remains an environment remainder |
