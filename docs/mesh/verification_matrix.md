...
| T5.1–T5.8 pack scope, activity, attribution | **Landed; inbound body now hydrates search/pack stubs** | Product commands + unit tests. Authorized BodyFetch replaces the `[ee.team.history]` stub with UTF-8 teammate text and enqueues a SingleDocument index job so `--memory-scope team` can recall it. Isolated 2026-08-15: `hydrate_inbound_team_memory_body_replaces_history_stub` 32m 07s compile + 63.77s test EXIT 0. | Two-human Tailscale search/pack e2e remains an environment remainder; TeamJoin loopback uses the same hydrate path |
...
32m 07s + 63.77s EXIT 0) so `--memory-scope team` can recall teammate text.
