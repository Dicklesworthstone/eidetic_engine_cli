...
| Versioned team-port migration | `bd-tc-followup-oo7d2.1` **shipped 2026-08-18** | First-class `ee team port show` / `ee team port migrate --to N --confirm`. Appends signed `teamPortMigrated` (does not rewrite genesis). Isolated CSS `/home/ubuntu/ee-mesh-campaign-verify-pinned/eidetic_engine_cli-port` at `10a00f7b` plus overlay: `cargo test --lib migrate_team_port` 2 passed in 43.52s (`folds_without_rewriting_genesis`, `rewrites_enrolled_peer_locator_and_leaves_pair_key`). Live CLI soak `/home/ubuntu/ee-port-migrate-soak-20260818` `PortMigrateSoak` `team_28a364dd67358ca114001f02b4` genesis `blake3:206247e353e9e586d380009b53fd34a9425cb9c5ba81f8b7d18f482082118223`: show gen 1 port 41888; migrate without `--confirm` usage exit 1; `--to 80` nonprivileged refuse; `--to 41888` same-port refuse; `--to 41999 --confirm` `migrated=true` `currentHelloPort=41999` `previousHelloPort=41888` `portGeneration=2` same genesis hash `pairKeysUnchanged=true` `grantsUnchanged=true`. Status `helloPort=41999`. Invite `helloPort=41999`. Doctor `broker_port` warning repairs with `ee team port migrate --to 41888 --confirm` or `EE_MESH_HELLO_PORT=41999`. |
