...
| T6.1 steward | **Proven** | Isolated 2026-08-14: `execute_team_steward_once_reapplies_a_missed_member_removal` 51.59s. Doctor/status stay read-only; daemon `TeamSteward` executes unless `--dry-run` | Network pair-retry still uses `ee mesh sync --once` after `ran_sync` |
