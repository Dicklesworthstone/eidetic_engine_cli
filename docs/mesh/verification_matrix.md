...
| Windows local control transport | `bd-tc-followup-oo7d2.9` **closed 2026-08-18** | Loopback TCP + owner-only `%LOCALAPPDATA%\eidetic-engine\mesh-responder.control` endpoint file (`ee.mesh.responder.control.endpoint.v1`). Unix UDS unchanged. Isolated CSS Unix `cargo test --lib responder_broker` 13 passed. SurfaceBookJE native 14 passed including `windows_control_endpoint_publishes_and_status_submits` and verbatim `\\?\` owner-safe path. Live soak `C:\Users\jeffr\ee-tc-win-control-soak` `WinControlSoak`: `hello-responder run --port 41889` bound `100.120.65.94:41889`; `status --json` `running=true` `listenAddress=100.120.65.94:41889` `degraded=[]`; `register --peer peer_8172d940cc571dda65183dd6325abbf2 --port 41889` `ok=true` `registeredRoutes=1`. CLI register was Unix-only (`PlatformUnsupported`) until the Windows handler compiled; owner-safe walk now strips `\\?\` so canonicalize()d paths pass. Named-pipe listen is rejected until a safe-Rust adapter exists (TC-D17); loopback TCP is the Windows control transport. |
