...
| T6.1 steward | **Proven** | Isolated 2026-08-14: membership, projects, orphaned Next pair, inbound memory rematerialize, SingleDocument index enqueue, and inbound body placeholders. `reconcile_inbound_team_memories_replays_allowed_import_ledger_events` 72.43s exit 0. Rematerialized stubs enqueue `sidx_*` so team search/pack do not wait on a full rebuild. Doctor `inbound_body_fetches` warns on `metadata_only` placeholders. | Live BodyFetch still needs mesh enabled + Body-lane Allow + reachable peers. Steward rematerializes placeholders; apply verifies fetched bytes. |
...
inbound-memory rematerialize, SingleDocument index enqueue, and
inbound body-placeholder publication with nonce-checked apply.
Isolated 2026-08-14T18:14–18:31Z: rematerialize 72.43s, project
78.90s, all exit 0. Isolated 2026-08-14T18:42–19:00Z: inbound
body placeholder 15m 33s compile + 132.74s test, exit 0.
