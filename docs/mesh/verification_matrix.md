...
| --- | --- | --- |
| Two-human Tailscale US-4 soak | `bd-tc-epic-qzk7o.3.8` (T2.6) | Isolated loopback/TeamJoin proven; `probe_mesh_capability` is Ready when enabled; smoke can require contact via `EE_E2E_REAL_TAILSCALE_REQUIRE_CONTACT=1`. Two distinct humans on a real tailnet still unproven |
...
| T5.9 Windows SID/DACL/reparse | **Adapter compiles; inbound TeamJoin TCP is no longer Unix-gated** | Isolated `cargo check --target x86_64-pc-windows-gnu --lib` Finished 25m 06s EXIT 0 after lifting TCP bind/accept/hello/EventFetch/BodyFetch off `cfg(unix)`. Tailscale LocalAPI and UDS control stay Unix. Adapter rejects reparse points, pins file identity, applies/verifies protected owner+SYSTEM DACL. Windows CI smoke now creates a local team through that store. | No Windows-host runtime DACL soak outside CI; Tailscale WhoIs remains Unix |
