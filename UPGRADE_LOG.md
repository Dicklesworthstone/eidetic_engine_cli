...
- [x] Publish and verify corrected 0.15.2 DSR binaries and Homebrew metadata;
...
- [x] Compile and test EE against the public corrected storage packages.
- [x] Pass the original native-model rerank and 60-operation writer probes
  before publishing EE 0.15.2.
- [x] Qualify all six final binaries, publish and verify the crate and assets,
...
The corrected EE 0.15.2 crate is published from
`1478b2f3b0a912d5302808f6dc6de9e56e589c05`, tagged `v0.15.2`.
All-target check, strict Clippy, formatting, the forbidden-dependency check,
and compilation of the extracted package passed. All 58 selected tests passed:
15 regressions, nine dependency contracts, 33 installer cases and the existing
real-model runtime regression. The installer rerun used the test-only source-pin
expectation correction in `6e99cedb`; that file is excluded from the crate, and
production code remains identical to the release source. The earlier stale-pin
failure is retained rather than relabeled as a passing invocation.

The public crate's SHA-256 is
`e4d5d5896b1628e3384ffe653414f6a0730961bf6c0adc06d7ab9e9c16b17706`.
It matches the qualified Cargo-generated candidate byte for byte, including all
339 source files, original manifest, lockfile and tagged VCS record. Publication
used Cargo's Registry Web API with metadata generated by Cargo, preserving that
exact archive after the excluded installer test changed on main. The public
index matches all 48 direct dependency declarations and ten features. A fresh
Cargo installation into an empty installation prefix passed all seven core
workflow checks; it used a warm dependency/build cache, with no sibling sources
or injected Git build metadata.

The final GNU binary passed the unchanged six-writer, 60-write workload in
27.79 seconds, including all eight persistence/index assertions. The original
160-request real-model rerank workload scored all 96 candidates permitted by
its selected portable profile in 13.966 seconds; the separate fusion-only
request returned all 160 permitted by its own selected profile. No runtime
budget or request count was raised. Three real Model2Vec retrieval scenarios
and installed hooks with automatic daemon model warming also passed.

The tagged shell installer passed an offline installation into a fresh retained
container in 6.255 seconds. Its doctor self-test, Bash completions and installed
binary hash/version/source checks passed. The original harness suppressed
cleanup to preserve files and containers; cleanup, network downloads and
signature verification were not qualified by this fixture.

All six final binaries passed memory workflows with their exact hashes,
version, source and target metadata verified. Apple Silicon and Windows ran
natively; Intel Mac used Rosetta and GNU/Linux ARM64 used QEMU on Debian 10.
GNU/Linux x86-64 and musl x86-64 also ran on Debian 10. Both GNU binaries passed
the glibc 2.28 symbol gate; musl has no interpreter or dynamic-library
dependencies. Windows additionally passed actual byte-lock contention and
two successive doctor repairs, preserving previous/latest pointers and
releasing its lock on process exit. Its cross-build retains platform-specific
unused-import and dead-code warnings; strict Clippy qualification was on Linux.

DSR run `248c90a7-fe8b-4590-b135-0956afc0386d` built the six binaries from the
same tagged source without GitHub Actions. Windows attempt one failed before
compilation on a registry-cache unpack collision; attempt two succeeded.
The five completed target results were reused without rebuilding or changing
their source. The original failed attempt, source stages and receipts remain.

GitHub release [v0.15.2](https://github.com/Dicklesworthstone/eidetic_engine_cli/releases/tag/v0.15.2)
was published as stable/latest on 2026-09-12 at 18:48:32 UTC. All 16 public
downloads match their prepared hashes, including both tagged installers;
each of the six archive payloads matches its qualified platform binary.
DSR regenerated `SHA256SUMS` from the six manifest archives; that checksum
inventory was verified separately, with the original preparation record retained.
The public manifest binds the release source and all 572 registry package
checksums, with seven supplemental upstream source revisions and private build
paths redacted. Assets remain unsigned.

The four-platform Homebrew formula was published in
[`3da22c6e1f42`](https://github.com/Dicklesworthstone/homebrew-tap/commit/3da22c6e1f426b26010e5d69c27d9cbb6ffff4a8)
and read back byte for byte from that commit and current main. All four archive
URLs and SHA-256 values match verified public assets; Ruby syntax passed.
An actual `brew install` was not run. Publication used no GitHub Actions or
repository dispatch; the Homebrew commit includes `[skip ci]`.
...
checks used a private frozen source graph; they do not substitute for the
final public-registry and release-binary checks recorded above.

The EE lockfile has 573 packages: EE itself and 572 checksum-pinned crates.io
packages, with no path/git dependencies or Cargo patches. The initial
path-to-registry conversion changed only source/checksum metadata; package
versions and dependency edges matched the preceding combined candidate.
The subsequent three-package FrankenSQLite 0.4.1 correction is recorded above.
There is exactly one Asupersync version, `0.5.0`.
